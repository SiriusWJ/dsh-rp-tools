  ...
  // ---- 本地生图：单张 ----
  {
    ...
    execute: async (args, exec) => {
      ...
      const result = await generateOne(cfg, {
        ...
        width: w,
        height: h,
        sizeKey: illustrateSizeKey(session, args.prompt, args),
        session,
      });
...
/** 用户外部导入的立绘放在会话目录的这个子目录里（`rp-sessions/<id>/portraits/`）。 */
const PORTRAIT_DIR = 'portraits';
/** 外部导入立绘的体积上限 —— 用户自己的图，挡的是手滑选了个几十 MB 的原图。 */
const PORTRAIT_MAX_BYTES = 8 * 1024 * 1024;
/** 只承认这三种（浏览器 img 都认），其余一律回 400，不猜扩展名。 */
const PORTRAIT_MIME_EXT = { 'image/png': 'png', 'image/jpeg': 'jpg', 'image/webp': 'webp' };
/**
 * 角色名 → 安全文件名（只把**路径有意义的字符**换掉，中文保留 —— 玩家要能在文件夹里认出来）。
 *
 * 为什么要 slug 而不是直接用名字：名字是用户/模型给的，里面可能有 `/`、`..`、`:` 之类，
 * 拼进路径就成了写入任意位置。**文件名由宿主生成**是这里唯一的安全边界（读取端还有第二道
 * 前缀校验，见 /rp-tools/portrait-image）。
 */
const portraitFileSlug = (name) => String(name ?? '').trim()
  .replace(/[\\/:*?"<>|\u0000-\u001f]/g, '_').replace(/\s+/g, '_').slice(0, 60) || 'portrait';
...
function installRoutes(ctx) {
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    // ── 立绘：用户**外部导入**的图片（用户要求「也支持用户用外部导入立绘」）──────
    // 前端把用户选的图读成 data URL 传上来（浏览器不能把本地路径交给服务端），
    // 我们落到本会话目录 `rp-sessions/<id>/portraits/<名字>.<ext>`，并把**会话内相对路径**
    // 记进 `portraits[name].imported`；渲染走下面的 /rp-tools/portrait-image。
    // 只承认 png/jpeg/webp、上限 8MB —— 这是用户自己的图，不是不可信输入，但仍要挡住
    // 路径穿越（文件名由我们生成）与超大 body。
    hostCtx.effect(() => hostCtx.webServer.register({
      kind: 'exact',
      ...
      handler: async (request, response) => {
        ...
        if (!sameOrigin(request)) { sendJson(response, 403, { ok: false, error: 'cross-origin request rejected' }); return; }
        try {
          const body = await readJsonBody(request, PORTRAIT_MAX_BYTES * 2);
          const rawId = String(body.sessionId ?? '').trim();
          if (!rawId) { sendJson(response, 400, { ok: false, error: 'sessionId is required' }); return; }
          const id = normalizeSessionId(rawId);
          const name = String(body.name ?? '').trim();
          ...
          const ext = PORTRAIT_MIME_EXT[m[1]];
          const buffer = Buffer.from(m[2].replace(/\s+/g, ''), 'base64');
          if (!buffer.length) { sendJson(response, 400, { ok: false, error: '图片内容为空' }); return; }
          if (buffer.length > PORTRAIT_MAX_BYTES) {
            sendJson(response, 413, { ok: false, error: `图片太大（${Math.round(buffer.length / 1024 / 1024)}MB，上限 8MB）` });
            return;
          }
          const lorePath = ensureSessionLore(id, body.workspace);
          const sessionDir = lorePath?.dir;
          if (!sessionDir) { sendJson(response, 400, { ok: false, error: '拿不到本会话的工作区目录' }); return; }
          const relFile = `${PORTRAIT_DIR}/${portraitFileSlug(name)}.${ext}`;
          const abs = join(sessionDir, relFile);
          ...
          session.portraits = {
            ...
            [name]: {
              ...
              imported: { file: relFile, bytes: buffer.length, at },
            },
          };
          const saved = saveSession(session);
          sendJson(response, saved ? 200 : 500, {
            ok: saved, sessionId: id, name, file: relFile, bytes: buffer.length,
            // `v`：同名重导时文件名不变，浏览器会命中 1 小时缓存 —— 带上时间戳才能换图即换。
            url: `/rp-tools/portrait-image?sessionId=${encodeURIComponent(id)}&name=${encodeURIComponent(name)}&v=${encodeURIComponent(at)}`,
