 ...
 * assets/<分类>/<id>.<ext>   库里的图（1.13.0 起真存了一份副本，所以包是自包含的）
 * cards/<slug>.{md,json,launch.md,png}  导入卡产物（含卡面与开局引导）
 * ```
 *
 * **不装**：`snapshots/`（那是快照自己，套娃没有意义），以及会话工作区里零散的图
 * （宿主的生图工具会把结果写到 `<工作区>/dsh-image-gen/`，那些原件不进包 ——
 *  已经用 `rp_assets(action:"import")` 收进 `assets/` 的在包里，没收的本来就只是过程产物）。
 *
