...
import { GLOBAL_TOOLS_DEFAULT, GLOBAL_TOOLS_ALLOW_MAX, groupGlobalTools, TOOL_NAME_RE, normalizeGlobalToolsAllow } from './global-tools-defaults.js';
...
/**
 * 「顺带看看会话作用域里有什么工具」用的样本 agent（`agent/created` / 装配时记下**最近**一个）。
 *
 * 为什么需要：`tools.schemas()` 省略 scope 只给**全局层**，而像 `web_search` / `web_fetch`
 * 这种宿主内置的工具是注册在**会话作用域**的 —— 于是设置页会把「一直在用」的
 * `web_search` 标成「未注册」，用户看到的是「明明是内置的，怎么说没装」。
 * 拿一个活着的 agent 调 `schemas(agent)` 就能拿到「该作用域可见的那一份」。
 *
 * 只留一个引用、不追所有 agent：工具是**按预设**注册的，同一预设下的 agent 看到的集合相同，
 * 随便一个样本就够（这也是开销最小的做法）。拿不到就退回纯全局视图（老行为，不报错）。
 */
let toolScopeSampleAgent = null;

/**
 * 列出本机全部工具名 = **全局层 ∪ 会话作用域里「本来就配过」的那几个**。
 * 拿不到注册表时返回 null（**不是空数组**）—— 界面要能区分「一个工具都没有」与「查不到」。
 *
 * ⚠️ **作用域那一份必须过滤**，不能照单全收（实测踩过，见下）：样本 agent 是「最近创建的
 * 那个」，在本机就是**用户当前这个对话的 agent** —— 它的作用域里有 `read` / `write` / `glob` /
 * `pwsh` / `todo_write` 这 26 个自己的工具。照单全收的话设置页会突然多出 26 行「未登记归属」，
 * 而那张表的语义是「**全局**注册的工具 / 要放行给 DM 的第三方工具」，不该出现这些。
 *
 * 所以口径是：**全局层**照收；作用域那一份**只收「配置里已经有的名字」**（`allow`）——
 * 这样 `web_search`（出厂默认名单里就有、注册在会话作用域）会正确显示成「本机有」，
 * 而 agent 自己的 `read` / `write` 之类不会跑进来。
 *
 * @param agent 可选的样本 agent；省略时用 {@link toolScopeSampleAgent}
 * @param allow 当前放行名单（只有出现在这里的会话作用域工具才补进清单）
 */
function listGlobalToolNames(agent = toolScopeSampleAgent ?? undefined, allow = null) {
  try {
    const tools = hostToolsLookup?.tools;
    const globalSchemas = tools?.schemas?.();
    if (!Array.isArray(globalSchemas)) return null;
    const names = new Set(globalSchemas.map((t) => String(t?.name ?? '')).filter(Boolean));
    const known = allow instanceof Set ? allow : new Set(Array.isArray(allow) ? allow : []);
    if (agent && known.size) {
      try {
        const scoped = tools.schemas(agent);
        for (const t of Array.isArray(scoped) ? scoped : []) {
          const name = String(t?.name ?? '');
          // ① 只收配置里已经点过名的（见上面的注释：不然 agent 自己的工具会涌进来）；
          // ② rp_* 是本插件注册在 dm 作用域的，**不是**「要放行给 DM 的第三方工具」，
          //    混进来会让用户看见一排自己插件的工具，还会以为勾它有什么用。
          if (name && known.has(name) && !name.startsWith('rp_')) names.add(name);
        }
      } catch { /* 作用域取不到就只报全局层（老行为） */ }
    }
    return [...names].sort();
  } catch { return null; }
}
...
/**
 * 工具名合法性（与宿主命名一致）与**归组**的实现都在共享模块 `global-tools-defaults.js` 里 ——
 * `groupGlobalTools` 现在从那儿 import（客户端冒烟测试的桩调用的是同一份，避免桩与实现走散）。
 */
...
const TOOL_HINTS = {
  ...
  web_fetch: '宿主内置的抓网页（与 web_search 同一组）；不联网可以一起关',
...
const rpTools = [
  ...
  // ---- 全局配置（卡库根目录 / 默认宏列表）----
  {
    ...
    execute: async (args) => {
      ...
      // 同一个口径：作用域那份只收以放行名单里点过名的（见 listGlobalToolNames 的注释）
      const available = listGlobalToolNames(undefined, toolView.allow);
      return {
        ok: true,
        action,
  ...
  try {
    ...
    const off = ctx.on('system-prompt/assemble', async (assembly, context, next) => {
      ...
      try {
        ...
        // 顺手把这次装配的 Agent 记成设置页的**样本**（见 toolScopeSampleAgent 的注释）。
        // 为什么非要有这一条：`apply()` 里的 agent/created 只在「新建会话」时响，插件是
        // **运行中重新装载**的（改完代码就是这个场景），已经在跑的会话不会再触发一次 ——
        // 那样样本会一直是空，`web_search` 就继续被标成「未注册」。装配每轮都发生，补得上。
        try {
          const agent = context?.agent ?? context?.scope;
          if (agent && typeof agent === 'object') toolScopeSampleAgent = agent;
        } catch { /* 取不到就还是靠 agent/created */ }
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        if (request.method === 'GET') {
          try {
            const cfg = loadStyles();
            const view0 = globalToolsAllowView(cfg);
            // 放行名单要**先**算出来：作用域那一份只收这里已经点过名的工具
            // （否则样本 agent 自己的 read/write/glob 会涌进设置页，见 listGlobalToolNames 的注释）。
            const all = listGlobalToolNames(undefined, view0.allow);
            const view = view0;
            const allowSet = new Set(view.allow);
            sendJson(response, 200, {
              ok: true,
              ...view,
              // 按**来源插件**归组（设置页一个插件一个勾选框）。
              // 组上带 `all` / `some` 两个现成判断，界面不用自己数：
              // 全勾 = 打开该插件全部工具；勾了一部分 = 部分（勾选框显示 indeterminate）。
              //
              // 本机没注册、但配置里有的名字（如 `web_search` 被注册在**会话作用域**，
              // 于是不在全局视图里）**混进它该在的那一组**，工具上带 `missing` 标记 ——
              // 以前它们自成一组「配置里有、本机没注册」，用户看到的是同一个插件的工具被劈成两处。
              // 一个都没有归属的（映射表还没跟上）才落进 `__unmapped__` 那一组。
              groups: all === null ? null : (() => {
                // 键必须稳定且唯一：整份「可见 + 已配置」的名单按同一口径归组。
                const missing = view.allow.filter((n) => !all.includes(n)).sort();
                const missingSet = new Set(missing);
                const groups = groupGlobalTools([...all, ...missing]).map((g) => ({
                  key: g.key,
                  label: g.label,
                  // 「这组是图片/生图插件」：映射表点名的，或整组工具名都像图片工具。
                  // 前者是主力（生图插件里有 view_canvas 这种看不出图片的名字）。
                  image: g.image === true,
...
export const __debug = {
  ...
  /** 测试用：塞一个「样本 agent」（生产由 apply 里的 agent/created 记下最近一个）。 */
  setToolScopeSampleAgent: (agent) => { toolScopeSampleAgent = agent ?? null; },
  /**
   * 测试用：直接调一次「本机工具名」的取并集逻辑（全局层 ∪ 会话作用域里**已点过名**的）。
   * `agent` 传 null 时用当前样本；`allow` 就是那份「已经点过名」的名单。
   */
  listToolNames: (agent, allow) => listGlobalToolNames(agent ?? undefined, allow),
...
export function apply(ctx) {
  ...
  // 记一个「样本 agent」给设置页用（见 toolScopeSampleAgent 的注释）：
  // `tools.schemas()` 不带 scope 只给全局层，会漏掉**会话作用域**注册的内置工具（如 web_search），
  // 于是设置页会把一直在用的 `web_search` 标成「未注册」。这里每次 agent 创建时换成本次这个 ——
  // 用最近的样本，装了新插件/换预设也跟得上。只存引用、不碰它的字段（活对象不许序列化），
  // 拿不到就退回纯全局视图（老行为，不报错）。
  //
  // 注册在 `apply`（宿主组合）而不是 dm 作用域：这里要观察的是**所有** agent，
  // 而它只用来给「本机有哪些工具」取一个样本，读到别的会话的工具名没有副作用。
  try {
    ctx.on('agent/created', (payload) => {
      const agent = payload?.agent ?? payload?.ctx?.agent;
      if (agent && typeof agent === 'object') toolScopeSampleAgent = agent;
    });
  } catch { /* 没有该事件就跳过（老宿主） */ }
  ...
  try {
    ctx.on('session/disposed', (session) => {
      try {
        const id = normalizeSessionId(session?.id ?? session?.header?.id);
        // 样本 agent 也要放掉：它属于这个会话，留着就是**长期持有一个已结束的会话对象**
        // （与下面这几行同一个理由）。下次 agent/created 会重新记一个。
        // ⚠️ 比 id 前先归一：agent.id 是 `session-<uuid>`，而 session/disposed 给的是裸 uuid
        //（工具侧与界面侧两种形式混用是本仓库的老坑，见 normalizeSessionId 的注释）。
        if (toolScopeSampleAgent && normalizeSessionId(toolScopeSampleAgent.id) === id) {
          toolScopeSampleAgent = null;
        }
