...
function renderDmSetup(session, { loreFile } = {}) {
  ...
  if (coverCard) {
    ...
    // 已经有真立绘（生成的）或已登记过卡面 → 不重复列，免得 DM 以为有两张图
    const ownerHasArt = Boolean(ownerArt && (ownerArt.card || ownerArt.generated?.file));
  ...
  if (arts.length) {
    lines.push('- **已有可用图：先在下面找，有就直接展示，不要再生成**（展示时把地址原样放进 `dsh-ui` 的 image 组件）：');
    lines.push(...arts);
  }
