...
import { buildImport, cardToMarkdown, cardToOpeningFile, buildOpeningPrompt, DEFAULT_USER_LABEL, localizeAttributes, listGreetings } from './card-import.js';
...
/**
 * 确保本会话有独立的世界书文件；老版本把世界书放在工作区根目录，这里做**一次性迁移**：
 * 旧文件存在而新文件不存在 → 复制一份给本会话（**旧文件原样保留**，不删不改）。
 *
 * 迁移后两个会话各有一份拷贝：内容仍然是混过的（历史事实无法自动拆分），
 * 但从这一刻起可以各自清理、互不影响。返回值里带 `migrated` 供界面提示。
 */
function ensureSessionLore(sessionId, hint) {
  const p = sessionLorePath(sessionId, hint);
  if (!p) return null;
  try {
    if (existsSync(p.file)) return { ...p, migrated: false };
    if (!existsSync(p.legacy)) return { ...p, migrated: false };
    mkdirSync(p.dir, { recursive: true });
    writeFileSync(p.file, readFileSync(p.legacy, 'utf8'), 'utf8');
    return { ...p, migrated: true };
  } catch {
    return { ...p, migrated: false };
  }
}
...
/**
 * 中和 ST / 宿主风格的 `{{…}}` 双花括号 —— **但放行我们注册过的宿主变量**。
 *
 * 为什么必须做：宿主渲染时会**严格插值** `{{变量}}`，未注册的名字直接抛错
 * （世界设定里写 `{{user}}`（ST 生态的习惯写法）曾经让整轮装配失败）。
 * 而我们现在通过 `ctx.systemPrompt.variable('user', …)` 注册了 `user`，
 * 所以它应当**原样留着**交给宿主插值（这才是 DSH 原生的做法，等价于酒馆那边的「身份宏」）；
 * 其余没注册的 `{{…}}` 仍然中和成全角，避免抛错。
 */
const HOST_PROMPT_VARS = ['user'];
function neutralizeMustache(text) {
  // 放行的变量先挖出来存好（用哨兵占位），否则后面的「把落单花括号也换成全角」会把它们一起换掉
  const keep = [];
  let out = String(text ?? '').replace(/\{\{\s*([^{}]{0,80}?)\s*\}\}/g, (m, name) => {
    const clean = String(name).trim();
    if (HOST_PROMPT_VARS.includes(clean.toLowerCase())) {
      keep.push(`{{${clean}}}`);
...
function installStandingPrompt(ctx, opts = {}) {
  try {
    ...
    // ── 身份宏：把 `{{user}}` 注册成**宿主变量**（DSH 原生的做法，等价于酒馆那边的身份宏）──
    // 宿主在 renderPrompt 时严格插值 `{{变量}}`，未注册的名字会抛错 —— 所以注册之后，
    // 用户在**世界设定 / 世界书条目里**写的 `{{user}}`（哪怕是我们导入之后才手写的）都能正确变成玩家称呼，
    // 而不是被 neutralizeMustache 换成全角括号。
    // 只注册 user：char 在一场戏里可能有多个角色，全局变量表达不了（导入时按卡名展开仍走 resolvePlaceholders）。
    if (typeof sys.variable === 'function') {
      try {
        const off2 = sys.variable('user', () => cardUserLabel(loadStyles()));
        ctx.effect(() => off2, 'rp-tools: user prompt variable');
      } catch (error) {
        console.warn('[rp-tools] 注册 {{user}} 变量失败:', error?.message ?? error);
      }
    }
    const off = ctx.on('system-prompt/assemble', async (assembly, context, next) => {
      ...
      try {
        ...
        const text = buildStandingText(session, { loreFile: sessionLorePath(sessionId)?.file });
        if (text) {
        ...
        if (turnText) {
          ...
          const entry = { name: TURN_CONTEXT, text: turnText };
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          sendJson(response, 200, {
            ...
            // 老版本的世界书在工作区根目录，被迁过来时提示一下（旧文件原样保留）
            migrated: lorePath?.migrated === true,
