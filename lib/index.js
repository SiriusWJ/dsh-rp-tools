...
import { buildImport, cardToMarkdown, cardToOpeningFile, buildOpeningPrompt, buildTidyPrompt, isDmCardData, dmCardToPrompt, DEFAULT_USER_LABEL, localizeAttributes, listGreetings, discoverMacros, collectCardText, MACRO_NAME_RE, AUTO_MACROS, autoMacroValue, isJunkLoreBody, loreKindOf } from './card-import.js';
 ...
 *
 * @returns {{ active: object[], dropped: object[], matchedByScan: number, emptySkipped: number }}
 */
function activateLore(entries, corpus, { sessionId, turn, maxEntries = LORE_MAX_ENTRIES, budgetChars = LORE_BUDGET_CHARS } = {}) {
  ...
  return { active, dropped, matchedByScan: matched.length, emptySkipped };
}
...
function buildTurnContext(session, corpus, { sessionId, turn } = {}) {
  ...
  // ② 世界书：被本轮对话触发的条目
  //    常驻条目**不在这里** —— 它们整段进 standing（见 renderConstantLore）：
  //    内容稳定，没必要每轮往会话历史里再追加一份全文。
  const entries = (sessionId ? loadLoreEntries(sessionId) : []).filter((e) => e.constant !== true);
  if (entries.length) {
 ...
 * 世界书的总览数字（面板顶部那行提示用）。
 *
 * `constantChars` 是**每轮都会注入**的字数 —— 用户问过「常驻条目到底占了多少上下文」，
 * 只报条数没法回答，所以这里把字也算出来。
 */
function loreOverview(entries) {
  const list = Array.isArray(entries) ? entries : [];
  const constant = list.filter((e) => e.constant);
  ...
  return {
    ...
    constantChars: constant.reduce((n, e) => n + e.title.length + e.body.length + 8, 0),
    kinds,
  };
}
 ...
 */
function renderConstantLore(entries) {
  const list = (Array.isArray(entries) ? entries : [])
    .filter((e) => e && e.constant === true && e.empty !== true && String(e.body ?? '').trim());
  if (!list.length) return '';
  return `【世界书·常驻】以下条目每轮都在，视为已确立的事实：\n${list.map((e) => `### ${e.title}\n${e.body}`).join('\n\n')}`;
}
...
function renderDmSetup(session, { loreFile } = {}) {
  ...
  if (arts.length) {
    lines.push('- 已有立绘（**不要重复生成**；要展示时把地址原样放进 `dsh-ui` 的 image 组件）：');
    lines.push(...arts);
  }
...
function buildStandingText(session, extra = {}) {
  ...
  const constants = renderConstantLore(extra.constants);
        ...
        if (request.method === 'POST') {
          ...
          try {
            ...
            sendJson(response, 200, {
              ...
              note: '只有命中的条目才进每轮上下文；空壳条目（正文只有模板残留）一律不注入。文件可直接编辑，DM 也能用 read/write 维护。',
            });
          } catch (error) {
            sendJson(response, 400, { ok: false, error: String(error?.message ?? error) });
          }
