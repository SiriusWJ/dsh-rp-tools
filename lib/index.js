...
import { GLOBAL_TOOLS_DEFAULT, GLOBAL_TOOLS_ALLOW_MAX, GLOBAL_TOOL_SOURCES, GLOBAL_TOOL_SOURCE_FALLBACK, TOOL_NAME_RE, normalizeGlobalToolsAllow } from './global-tools-defaults.js';
 ...
 *
 * 宿主不提供工具归属（见 `GLOBAL_TOOL_SOURCES` 的注释），所以这里查那张映射表；
 * 未命中的落进「其它」组。组内按名字排序，组间按 `GLOBAL_TOOL_SOURCES` 的声明顺序，
 * 「其它」永远排最后 —— 界面顺序稳定，用户扫一眼就能定位。
 *
 * @param names 工具名数组（来自 {@link listGlobalToolNames}）
 * @returns `[{ key, label, tools, image? }]`（只含非空组）
 */
function groupGlobalTools(names) {
  const rest = [];
  const out = [];
  for (const src of GLOBAL_TOOL_SOURCES) {
    const tools = names.filter((n) => src.match.test(n));
  ...
  const others = names.filter((n) => !rest.includes(n)).sort();
  if (others.length) out.push({ ...GLOBAL_TOOL_SOURCE_FALLBACK, tools: others });
  return out;
}
        ...
        if (request.method === 'GET') {
          try {
            ...
            sendJson(response, 200, {
              ...
              // 未注册的名字（配置里有、本机没有）单独一组列在最后，也要能取消 ——
              // 否则用户看不到自己配了什么，只能猜「为什么没生效」。
              groups: all === null ? null : (() => {
                const groups = groupGlobalTools(all).map((g) => ({
                  key: g.key,
                  label: g.label,
                ...
                if (missing.length) {
                  groups.push({
                    // 键不能和「其它」撞（两组的 key 都是 __other__ 的话，界面按 key 渲染会重影）
                    key: '__missing__',
                    label: '配置里有、本机没注册',
                    missing: true,
                    tools: missing.map((name) => ({ name, checked: true, imageLike: looksLikeImageTool(name), hint: '' })),
                  });
                }
