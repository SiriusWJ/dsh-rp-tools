    ...
    execute: async (args, exec) => {
      ...
      return {
        ...
        note: `${result.styleLabel} · ${result.width}×${result.height} · cfg ${result.cfg} · promptId=${result.promptId}`
          ...
          + (firstAsset ? ` · 已存进资源库（id=${firstAsset.id}，${ASSET_KIND_LABEL[firstAsset.kind]}）` : ''),
      };
    },
  },
  ...
  // ---- 资源库：浏览本会话已有的图，找到就复用，别重出 ----
  {
    ...
    execute: async (args, exec) => {
      ...
      if (action === 'tag') {
        ...
        if (!found) throw new Error(`rp_assets: 没有 id=${id} 这条资源`);
        return {
      ...
      if (action === 'get') {
        ...
        const one = assets.find((a) => a.id === want) ?? loadAssets(sid).assets.find((a) => a.id === want);
        if (!one) throw new Error('rp_assets: action:"get" 需要 id（先 list 拿到）');
        return {
...
function buildStandingText(session, extra = {}) {
  ...
  if (extra.loreFile) {
    ...
    parts.push(`【世界书】${extra.loreFile}`
      + (loreExists
        ? '\n（本会话专属；用 read 读、用 write/edit 增改。只有被对话触发的条目才会进上下文。）'
