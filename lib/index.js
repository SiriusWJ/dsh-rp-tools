...
const WORKFLOW_TEMPLATES = {
  ...
  /** NetaYumev35 无限制、未过滤极致二次元动漫插画模板。 */
  uncensored_anime: (st) => {
    const lora = st.lora ? String(st.lora) : '';
    const nodes = {
      '1': { class_type: 'CheckpointLoaderSimple', inputs: { ckpt_name: 'NetaYumev35_pretrained_all_in_one.safetensors' } },
      ...
      '5': {
        class_type: 'KSampler',
        inputs: {
          model: [lora ? '8' : '1', 0], seed: 0, steps: 20, cfg: 5.5,
          sampler_name: 'euler_ancestral', scheduler: 'normal',
          positive: ['2', 0], negative: ['3', 0], latent_image: ['4', 0], denoise: 1,
        },
      },
      '6': { class_type: 'VAEDecode', inputs: { samples: ['5', 0], vae: ['1', 2] } },
      '7': { class_type: 'SaveImage', inputs: { images: ['6', 0], filename_prefix: 'rp_uncensored_anime' } },
    };
    if (lora) {
      nodes['8'] = {
        class_type: 'LoraLoader',
        inputs: { model: ['1', 0], clip: ['1', 1], lora_name: lora, strength_model: Number(st.loraStrength ?? 0.8), strength_clip: Number(st.loraStrength ?? 0.8) },
      };
      // 重定向 KSampler 的 model 引用，以及 LoRA 的 clip 引用
      nodes['5'].inputs.model = ['8', 0];
      nodes['2'].inputs.clip = ['8', 1];
    }
    return nodes;
  },
  /** FLUX.1-Dev 顶级无限制、未过滤极致写实模板。 */
  uncensored_real: (st) => {
    const lora = st.lora ? String(st.lora) : '';
    const nodes = {
      '1': { class_type: 'UNETLoader', inputs: { unet_name: 'flux1-krea-dev_fp8_scaled.safetensors', weight_dtype: 'default' } },
      ...
      '6': {
        class_type: 'KSampler',
        inputs: {
          model: [lora ? '7' : '1', 0], seed: 0, steps: 20, cfg: 1,
          sampler_name: 'euler', scheduler: 'simple',
          positive: ['4', 0], negative: ['10', 0], latent_image: ['5', 0], denoise: 1,
        },
      },
      '8': { class_type: 'VAEDecode', inputs: { samples: ['6', 0], vae: ['3', 0] } },
      '9': { class_type: 'SaveImage', inputs: { images: ['8', 0], filename_prefix: 'rp_uncensored_real' } },
    };
    if (lora) {
      nodes['7'] = {
        class_type: 'LoraLoaderModelOnly',
        inputs: { model: ['1', 0], lora_name: lora, strength_model: Number(st.loraStrength ?? 0.8) },
      };
    }
    return nodes;
  },
...
/** 图像尺寸的三条槽位（顺序就是设置页那三行）。 */
const IMAGE_SIZE_SLOTS = ['scene', 'portrait', 'item'];
/** 出厂的全局图像尺寸（设置页「图像」里那三行）。
 *
 * 1.12.8 起调小：本地 ComfyUI 出图时间基本正比于像素数（本仓库实测 1024² 13–18s、
 * 1344×768 25s），而聊天里的图最多也就渲染到几百 px 宽 —— 出 1024 宽的图纯属浪费。
 * 现在：场景 768×432（≈0.33MP）、立绘 512×768（≈0.39MP）、道具 512×512（≈0.26MP），
 * 单张大概从 15~25s 降到 8~14s。
 */
const DEFAULT_IMAGE_SIZES = { scene: [768, 432], portrait: [512, 768], item: [512, 512] };
/** 降尺寸前的出厂值：老配置里还是这些值（= 从没自己改过）就跟着换成新的。 */
const LEGACY_DEFAULT_IMAGE_SIZES = { scene: [1024, 576], portrait: [640, 896], item: [768, 768] };
/** 最后的兜底：全局与风格都没给时用它（老模板的尺寸）。 */
const FALLBACK_IMAGE_SIZES = { scene: [1344, 768], portrait: [768, 1024], item: [1024, 1024] };
function defaultStyles() {
  ...
  return {
    ...
    defaultStyle: 'uncensored_anime',   // 用户实际在用的那套二次元
    ...
    // **全局图像尺寸**：三个用途各一对宽高（场景 / 立绘 / 道具）。
    // 以前尺寸藏在每个风格里（界面上只能看不能改），现在提到全局统一编辑 ——
    // 风格之间尺寸本来就一样，没必要逐个维护；要单风格特例仍可手写 styles.json 里的 sizes。
    imageSizes: JSON.parse(JSON.stringify(DEFAULT_IMAGE_SIZES)),
    styles: {
      // 三个内置风格就是用户实际用的那三套（workflow 直接用他自己的模板名）
      uncensored_anime: mk('二次元', 'tone001_manga.safetensors',
        'masterpiece, finest quality, highly detailed anime illustration, 8k',
        '基于顶级 NetaYumev35 动漫大底模和漫画网点 LoRA 搭建，100% 物理无限制，未过滤。高精度二次元/漫画原画风。',
        { workflow: 'uncensored_anime', cfg: 5.5, steps: 20, sizes: { scene: [768, 432], portrait: [512, 768], item: [512, 512] } }),
      uncensored_real: mk('写实', null,
        'hyperrealistic photography, highly detailed skin texture, raw photo, realistic lighting, dramatic cinematic composition, 8k, professional color grading',
        '基于顶级 FLUX.1-Dev 蒸馏底模与 DualCLIP（t5xxl + clip_l）搭建。100% 物理无限制，对复杂肢体、写实质感、物理场景还原度极高。',
        { workflow: 'uncensored_real', cfg: 1, steps: 20, sizes: { scene: [768, 432], portrait: [512, 768], item: [512, 512] } }),
      manga: mk('黑白漫画', null,
        'black and white manga panel, screentone shading, crisp ink lineart, high contrast, comic composition',
        '通用黑白漫画分镜（krea2 模板，无 LoRA，靠提示词）'),
    },
  };
}

/**
 * 已经**移除**的内置风格：用户要求风格库只留「二次元 / 写实 / 黑白漫画」，
 * 而「二次元 / 写实」用他自己那两套（`uncensored_anime` / `uncensored_real`）——
 * 所以老的 9 个 krea2 内置风格、以及我中途加过的 `anime` / `realistic` 都要从老配置里清掉。
 */
/** 老的尺寸预设（迁移时用来判断「这个值是不是我们当年给的默认值」）。 */
const LEGACY_STYLE_SIZES = [[1344, 768], [1024, 1024], [768, 1024], [1024, 576], [640, 896], [768, 768]];

const REMOVED_BUILTIN_STYLES = [
...
function loadStyles() {
  ...
  if (!cfg.cards || typeof cfg.cards !== 'object') {
    cfg.cards = { root: '', userLabel: DEFAULT_USER_LABEL, macros: { user: DEFAULT_USER_LABEL }, macrosSeeded: true };
  } else {
  ...
  // 老配置迁移：风格库只留「二次元 / 写实 / 黑白漫画」，其余内置风格清掉
  {
    let dropped = false;
    for (const key of REMOVED_BUILTIN_STYLES) {
      if (cfg.styles?.[key]) { delete cfg.styles[key]; dropped = true; }
    }
    // 默认风格若指向被删的键 → 用第一个可用风格（否则生图会因为找不到风格报错）
    if (!cfg.styles?.[cfg.defaultStyle]) {
      cfg.defaultStyle = Object.keys(cfg.styles ?? {})[0] ?? 'manga';
      dropped = true;
    }
    // 内置风格的尺寸调小了：老配置里若还是**旧的预设值**就换成新的（用户自己改过的不动）
    const DEFAULTS = defaultStyles().styles;    for (const [key, st] of Object.entries(cfg.styles ?? {})) {
      const target = DEFAULTS[key]?.sizes;
      if (!target || !st || typeof st !== 'object') continue;    // 只动内置风格
      for (const slot of ['scene', 'portrait', 'item']) {
        const cur = st.sizes?.[slot];
        if (!Array.isArray(cur) || cur.length !== 2) continue;
        const legacy = LEGACY_STYLE_SIZES.some(([w, h]) => Number(cur[0]) === w && Number(cur[1]) === h);
        const bigger = Number(cur[0]) * Number(cur[1]) > target[slot][0] * target[slot][1];
        if (legacy && bigger) { st.sizes = { ...st.sizes, [slot]: target[slot] }; dropped = true; }
      }
    }
    if (dropped) saveStyles(cfg);
  }
  // 老配置迁移：**全局图像尺寸**（1.9.1 起尺寸从风格里提到全局统一编辑）
  {
    let fixed = null;
    const src = cfg.imageSizes && typeof cfg.imageSizes === 'object' ? cfg.imageSizes : {};
    const out = {};
    for (const slot of IMAGE_SIZE_SLOTS) {
      const cur = normalizeSizePair(src[slot]);
      // 「降尺寸」也走这里：**只有还等于旧出厂值**（= 用户没动过）才换成新出厂值 ——
      // 自己调过尺寸的人不该被静默改掉（§9 不替用户猜）。
      const oldValue = LEGACY_DEFAULT_IMAGE_SIZES[slot];
      const untouched = cur && cur[0] === oldValue[0] && cur[1] === oldValue[1];
      const pair = (untouched ? null : cur) ?? DEFAULT_IMAGE_SIZES[slot];
      out[slot] = pair;
      if (!cur) fixed = true;
      else if (untouched) fixed = true;
    }
    if (!cfg.imageSizes || typeof cfg.imageSizes !== 'object' || fixed) { cfg.imageSizes = out; saveStyles(cfg); }
  }
  return cfg;
}
...
// 会话级配置（按 session 隔离）
// -------------------------------------------------------------------
// 全局：风格库、ComfyUI 地址、全局默认风格、**负面词**（styles.json）。
// 会话级：角色卡 / 世界设定 / 提示词前缀 / 风格备注 / 战役名 / 会话默认风格 / 随机表。
//         —— 注意：负面词**只属于全局**，会话里没有它（旧版本残留已清掉）。
...
function loadSession(sessionId) {
  ...
  return {
    ...
    // 负面词不属于会话配置（全局在 styles.json），这里不再留有它的位置。
    campaign: {
      name: '',
      prompt_prefix: '',
    ...
    // ── DM 设定（本会话的 DM 自己的规则 + 生图开关）─────────────────────
    // 为什么要它：DM（旁白）卡不是角色卡 —— 它的内容是「这个 DM 怎么带团」，
    // 早先会被导成一条名叫 DM 的角色（用户实测：第一条角色卡里全是 DM 预设）。
    // 这里既收卡导入的正文，也收用户手写的；`images` 决定要不要自动配图（会话隔离）。
    dm: (() => {
      const raw = data.dm && typeof data.dm === 'object' ? data.dm : {};
      const img = raw.images && typeof raw.images === 'object' ? raw.images : {};
      ...
      return {
        ...
        migrated: prompt !== '' && !(typeof raw.prompt === 'string' && raw.prompt.trim())
          ...
          : [],
        images: {
          // 默认**开**：用户明确要求「本地生图可用时，角色第一次出场要配图、重要场景也要」
          enabled: bool(img.enabled, true),
          firstAppearance: bool(img.firstAppearance, true),
          keyScenes: bool(img.keyScenes, true),
        },
      };
    })(),
 ...
 */
function buildTurnContext(session, corpus, { sessionId, turn, diagnostics } = {}) {
  const parts = [];
...
/**
 * fork 一个新会话时，把父会话的 RP 配置（世界 / 角色卡 / 随机表 / 前缀 / 风格备注 / 战役名 /
 * 会话默认风格）复制过去。
 *
...
/**
 * 把一次生成结果**记成某个角色的立绘**（会话配置里 `portraits[name].generated`）。
 *
 * 为什么要自动记：DM 给角色出第一次出场的图时，那张图就是它的立绘 —— 以前只有**面板**上
 * 点「立绘」才会落盘，于是 DM 出的图只活在聊天里：角色行还是占位头像、下一轮
 * 【本会话设定】的「已有可用图」里也没有它，DM 可能又出一张（用户实测的意思就是
 * 「第一次出场生成了立绘，可以给角色卡用」）。现在出图即登记，下一轮就能复用。
 *
 * 边界：**只在还没有立绘时写**（不覆盖面板里已有的那张，也不覆盖用户自己配的），
 * 且必须三要素齐全（只存 URL 的话换个 origin 就失效）。
 *
 * @returns {boolean} 是否真的写入了
 */
function recordGeneratedPortrait(sessionId, name, files, { style = '', elapsedMs = 0 } = {}) {
  try {
    const who = String(name ?? '').trim();
    const file = files?.[0];
    if (!who || !file?.file) return false;
    const session = loadSession(sessionId);
    const portraits = { ...(session.portraits ?? {}) };
    const current = portraits[who];
    if (current && typeof current === 'object' && current.generated?.file) return false;   // 已有立绘，不动
    portraits[who] = {
      ...(current ?? {}),
      generated: { file: String(file.file), subfolder: String(file.subfolder ?? ''), type: String(file.type ?? 'output') },
      style: String(style ?? ''),
  ...
  } catch { return false; }   // 记不上不影响这次出图本身
}

/**
 * 这一张该按哪个尺寸档出？
 *
 * **立绘默认纵向**（用户要求）：不指定尺寸时，如果画面里只认得出**一个已登记角色**
 * 且 TA 还没有立绘 —— 这一张按定义就会成为 TA 的立绘（见 `recordGeneratedPortrait`），
 * 那就用 `portrait`（纵向 512×768），否则头像会是一张横向场景图，在角色卡编辑器里
 * 只占半列、显得很小。
 *
 * 调用方显式给了 `width`/`height`/`aspect` 就一律不干预（`scene` 在这里只是「不指定」的意思）。
 */
function illustrateSizeKey(session, prompt, args = {}) {
  if (args.width || args.height || args.aspect) return 'scene';
  const who = characterInPrompt(session, prompt);
  if (!who) return 'scene';
  const entry = session?.portraits?.[who];
  const has = Boolean(entry && typeof entry === 'object'
    && (entry.generated?.file || entry.imported?.file || entry.card));
  return has ? 'scene' : 'portrait';
}
 ...
 *
 * ⚠️ **只用本会话的数据**：早先这里在会话没角色时会回退到全局 `campaign.character_sheet` /
 * `campaign.prompt_prefix`（styles.json 里的老字段）—— 那等于把别的战役的角色外观锚点
 * 塞进本次出图（跨会话串味）。现在没有会话数据就是没有，绝不回退到全局。
 */
function buildPrompt(cfg, st, prompt, session) {
  const parts = [];
  const prefix = session?.campaign?.prompt_prefix || '';
  ...
  const sheet = (Array.isArray(session?.characters) ? session.characters : []);
...
function aspectToSize(aspect, megapixels = 0.6) {
 ...
 *
 * 只在**恰好命中一个**角色时返回它的名字：
 * 多个角色同框时，画面主体到底是谁说不清，硬套其中一个人的种子没有意义，也不公平。
 */
function characterInPrompt(session, prompt) {
  const text = String(prompt ?? '');
  const names = (Array.isArray(session?.characters) ? session.characters : [])
...
/**
 * 角色固定种子：同一个角色每次出图都从同一份初始噪声起步。
 *
 * 为什么有效：种子决定初始噪声，是「长相漂移」的主要来源之一。固定之后，
 * 同一角色的脸型/五官分布会明显稳定，而姿势、表情、场景仍随提示词变化。
 * （要真正锁住五官需要参考图方案，见 docs/REFERENCE-COMPARISON.md §8.4；
 *   这一版是零成本的那一半。）
 *
 * 种子只用**角色名**推导、不含轮次号或时间 —— 那正是「固定」的意义。
 * 复用 rp_random 的同一套 PRNG，所以这个值既稳定又和骰子工具同源。
 */
function characterSeed(session, prompt) {
  const name = characterInPrompt(session, prompt);
  if (!name) return undefined;
  return hashSeed(`character:${name}`) % 2147483647;
}
...
async function generateOne(cfg, { prompt, style, seed, width, height, timeoutMs, sizeKey = 'scene', session }) {
  ...
  // 没显式给 seed 时：画面里只有一个已登记角色 → 用该角色的固定种子（跨场景保持一致）
  const pinnedName = characterInPrompt(session, prompt);
  const effectiveSeed = (seed === undefined || seed === null || seed === '')
    ? characterSeed(session, prompt)
    : seed;

  let w = width; let h = height;
  if (!w || !h) {
    const preset = resolveImageSizes(cfg, st, sizeKey);
    w = w ?? preset[0];
    h = h ?? preset[1];
  }

  const negative = cfg?.negative ?? '';
  const workflow = buildWorkflow(st, { prompt: fullPrompt, negative, width: w, height: h, seed: effectiveSeed });
  ...
  return {
    ...
    // 原始文件描述符（不带 origin）：界面要把它**记进会话配置**，下次打开面板时用同样的
    // 三要素重新拼出媒体 URL。只存 URL 的话换个 origin 就失效，所以存这三样。
    files: images.map((img) => ({
      file: String(img.filename ?? ''),
      subfolder: String(img.subfolder ?? ''),
      type: String(img.type ?? 'output'),
    })),
    // 让结果可解释：这轮用的是不是角色固定种子、seed 值是多少（便于复现或手动改）
    pinnedCharacter: pinnedName || '',
    seed: effectiveSeed === undefined ? null : effectiveSeed,
  };
}
...
const rpTools = [
  ...
  {
    name: 'rp_illustrate',
    description: '用本地 ComfyUI 生成一张图，风格可选（不传用默认风格）。通用出图工具：跑团/DM 配图、场景、NPC 立绘、道具线索、氛围图都走它。返回的图片链接会渲染成聊天里的卡片。不传尺寸时：画面里只提到一个已登记角色 → 出**纵向立绘**（角色卡用的比例），否则出场景横幅。',
    parameters: {
      ...
      aspect: { type: 'string', description: '宽高比，如 "16:9" / "3:4" / "1:1"；不传时按上面说的自动选（角色单人=纵向立绘，其余=场景）' },
      width: { type: 'number', description: '显式宽度（覆盖自动选档、aspect 与风格默认）' },
      height: { type: 'number', description: '显式高度（覆盖自动选档）' },
      label: { type: 'string', description: '给这张图起个短名字存进资源库（如「雨夜客栈大堂」）。**强烈建议填** —— 下次想复用同一张图就靠它找回来，不填就只能靠提示词原文模糊搜。' },
      tags: { type: 'string', description: '检索标签，逗号分隔（如「客栈,雨夜,室内」）。下次要「再放一张那个客栈的图」时按标签查得到，不用重新出。' },
      kind: { type: 'string', description: '资源分类：portrait（角色）/ scene（场景）/ item（道具）/ other。不传时按自动尺寸档推断（单人立绘=portrait，其余=scene）' },
    },
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          ...
          note: { type: 'string', required: true },
          // 以字符串输出：没有固定种子时给空串，省得为「number | null」写 oneOf
          seed: { type: 'string', required: true },
          pinnedCharacter: { type: 'string', required: true },
      ...
      render: (args, value) => {
        ...
        if (value.pinnedCharacter) lines.push(`  seed ${value.seed}（角色「${value.pinnedCharacter}」的固定种子，同一角色跨场景保持一致）`);
        return [{ type: 'text', text: lines.join('\n') }];
      },
    },
    /**
     * **并发安全**：本地出图一张 15~25 秒，模型一轮里发多个 `rp_illustrate` 时宿主会把它们
     * 放进并行池（最多 10 个在飞）。不声明的话宿主**默认串行**（`executionMode` 只认
     * `isConcurrencySafe(args) === true`，其余一律 exclusive）—— 那样「一次发三张」还是
     * 一张一张等，白白多花两倍等待。
     *
     * 凭什么算安全：这个 execute 只读配置/会话，然后 POST /prompt 拿 promptId、轮询
     * `/history/<promptId>` —— 每次调用各自一个 promptId，没有共享可变状态，ComfyUI 侧
     * 本来就是队列（并行只是把排队交给它，省掉的是模型往返）。
     */
    ...
    execute: async (args, exec) => {
      ...
      if ((!w || !h) && args.aspect) {
        const size = aspectToSize(args.aspect, 0.6);
        if (size) { w = w ?? size[0]; h = h ?? size[1]; }
      }
      const sizeKey = illustrateSizeKey(session, args.prompt, args);
      ...
      // 第一次给某个角色出的图 = 它的立绘：直接记进角色卡（下次复用，不再重复生成）。
      // 画面里认不出唯一角色时不记 —— 群像/场景图不该顶替某个人物的立绘。
      const portraitSaved = recordGeneratedPortrait(sessionIdOf(exec), result.pinnedCharacter, result.files, {
        style: result.styleLabel, elapsedMs: result.elapsedMs,
      });
      // 进资源库（真存一份字节）：下次要「再放一张那个客栈的图」时能按标签找回来，不必重出。
      // kind 优先用模型报的；没报就按自动尺寸档推断（单人立绘=portrait，其余=scene）。
      const archived = await archiveGeneratedImages(cfg, sessionIdOf(exec), result, {
        kind: ASSET_KINDS.includes(String(args.kind)) ? String(args.kind) : (sizeKey === 'portrait' ? 'portrait' : 'scene'),
        label: String(args.label ?? '').trim() || String(args.prompt ?? '').slice(0, ASSET_LABEL_MAX),
        tags: args.tags,
        characters: charactersInPrompt(session, args.prompt),
      });
      const firstAsset = archived[0];
      return {
        ok: true,
        ...
        media: result.media,
        seed: result.seed === undefined || result.seed === null ? '' : String(result.seed),
        pinnedCharacter: result.pinnedCharacter ?? '',
        assetId: firstAsset ? firstAsset.id : '',
        note: `${result.styleLabel} · ${result.width}×${result.height} · cfg ${result.cfg} · promptId=${result.promptId}`
          + (result.pinnedCharacter ? ` · seed ${result.seed}（角色「${result.pinnedCharacter}」固定种子）` : '')
          + (portraitSaved ? ` · 已记成「${result.pinnedCharacter}」的立绘（下次直接用这张，不必重出）` : '')
          + (firstAsset
            ? (firstAsset.deduped
              // 去重命中要说出来：否则模型看到「这张已经在库里」会以为出图没生效，可能再出一张
              ? ` · 这张与库里已有的 id=${firstAsset.id}（${ASSET_KIND_LABEL[firstAsset.kind]}）字节完全相同，没有重复存`
              : ` · 已存进资源库（id=${firstAsset.id}，${ASSET_KIND_LABEL[firstAsset.kind]}）`)
            : ''),
      };
    },
  },
  ...
  // ---- 生成工具：角色设定表管理（+ 可选出立绘）----
  {
    name: 'rp_character',
    description: [
      '管理战役的角色设定表，并按需用默认风格出一张角色立绘。',
      '角色名一旦登记，任何 rp_illustrate 的画面描述里提到它就会自动补上外观——这是保持角色长相一致的主要手段。',
    ...
    parameters: {
      ...
      appearance: { type: 'string', description: '外貌描述。按「发型颜色→眼睛→肤色体型→身高→日常穿着→标志性配饰」顺序写，生图时自动补进提示词' },
    ...
    execute: async (args, exec) => {
      ...
      if (args.portrait) {
        ...
        // 同 rp_illustrate：出图即登记成这个角色的立绘（下次复用）
        const saved2 = recordGeneratedPortrait(sessionIdOf(exec), String(target.name ?? ''), result.files, {
          style: result.styleLabel, elapsedMs: result.elapsedMs,
        });
        // 同时进资源库：立绘也是「本会话用过的图」，面板图墙要能看到、能换成场景图或再提回来
        await archiveGeneratedImages(cfg, sessionIdOf(exec), result, {
          kind: 'portrait', label: target.name, characters: [String(target.name ?? '')],
          group: '', prompt: `${target.name} 的半身立绘`,
        });
    ...
    parameters: {
      ...
      party: {
        ...
        description: '队伍成员状态，每项 {character, status, abilities, inventory, conditions, goal}。'
          + '`abilities`（技能/能力/专长）与 `inventory`（持有/装备）都是**会变**的当前值，'
          + '学到/失去/换装/用掉就更新这里，别写回人物卡。只列**你要更新的人**；传空数组即清空全部。',
      },
      flags: {
        type: 'object',
  ...
  // ---- 生成工具：会话级设置（世界 / 前缀 / 风格备注 / 战役名 / 会话默认风格）----
  {
    name: 'rp_session',
    description: [
      '查看或修改「当前会话」的 RP 设置（按会话隔离，互不影响）：世界设定、提示词前缀、会话默认风格、风格备注、战役名。',
      'DM 用它在开局录入世界观与基调；这些设置只作用于本会话（全局负面词与风格库用 rp_config）。',
      '',
    ...
    parameters: {
      ...
      images_enabled: { type: 'boolean', description: '本会话自动配图总开关（false = 不要主动调 rp_illustrate）' },
      images_first_appearance: { type: 'boolean', description: '角色第一次出场是否配立绘（配合 images_enabled）' },
      images_key_scenes: { type: 'boolean', description: '重要场景是否配氛围图（配合 images_enabled）' },
    },
    output: {
      schema: {
        type: 'object',
    ...
    execute: async (args, exec) => {
      ...
      if (action === 'set') {
        ...
        // DM 设定 + 本会话生图策略：与面板共用同一份会话配置（面板软刷新就能看到 Agent 改了什么）。
        // 只改传进来的那一个键，没传的保持原值 —— 否则 DM 设一次生图开关会把 DM 正文清空。
        if (args.dm_prompt !== undefined) {
          session.dm = { ...(session.dm ?? {}), prompt: String(args.dm_prompt), migrated: [] };
        }
        const imageFlags = [
          ['images_enabled', 'enabled'],
          ['images_first_appearance', 'firstAppearance'],
          ['images_key_scenes', 'keyScenes'],
        ].filter(([param]) => args[param] !== undefined);
        if (imageFlags.length) {
          const images = { enabled: true, firstAppearance: true, keyScenes: true, ...(session.dm?.images ?? {}) };
          for (const [param, key] of imageFlags) images[key] = args[param] === true;
          session.dm = { ...(session.dm ?? {}), images };
        }
      ...
      const lines = [
        ...
        `本会话生图：${session.dm?.images?.enabled === false ? '关' : '开'}`
          + `${session.dm?.images?.enabled === false ? '' : `（首次出场 ${session.dm?.images?.firstAppearance === false ? '关' : '开'}｜重要场景 ${session.dm?.images?.keyScenes === false ? '关' : '开'}）`}`,
  ...
  // ---- 生成工具：按场景文件批量 ----
  {
    name: 'rp_scenes',
    description: [
      '按场景文件（`scenes_*.json` 结构：**`scenes[].panels[]`**，每格有 positive/negative/seed/width/height）逐格批量生图。',
      '适合一次性出一整幕。单格失败会跳过并记录，不中断整批。',
      '',
      '**字段名是固定的**，写错会筛不到（真机复测踩过）：幕的 id 字段是 **`scene_id`**（不是 `id`、不是 `sceneId`），',
      '分镜 id 是 **`panel_id`**。最小合法示例：',
      '```json',
      '{"scenes":[{"scene_id":"S01","panels":[{"panel_id":"p1","positive":"雨夜客栈大堂","panel_type":"wide","seed":123}]}]}',
      '```',
      '`sceneId` 参数筛的就是上面的 `scene_id`（也认 `sceneId` / `id` / `title` 这类常见写法）；',
      '**筛不到会直接报错并列出文件里实际有哪些 id**，不会静默返回 0 张。',
    ].join('\n'),
    parameters: {
      scenesFile: { type: 'string', required: true, description: '场景 JSON 路径（绝对路径，或相对当前工作目录）' },
      sceneId: { type: 'string', description: '只处理某一幕：匹配场景对象的 **`scene_id`**（也认 `sceneId` / `id` / `title`），如 "S01"；不传则全部。筛不到会报错并列出可用的 id。' },
      style: { type: 'string', description: '风格 key；不传用默认风格' },
      limit: { type: 'number', description: '最多生成几张（调试用）' },
      label: { type: 'string', description: '整幕的名字（存进资源库存成「<label> · <场次/分镜>」，如「第三幕 追击」）' },
      tags: { type: 'string', description: '整幕共享的检索标签，逗号分隔（如「客栈,雨夜」）' },
    },
    output: {
      schema: {
        type: 'object',
    ...
    execute: async (args, exec) => {
      ...
      const wanted = scenes.filter((s) => !args.sceneId || String(sceneIdOf(s)) === String(args.sceneId));
      // **筛选不中必须报错**（真机复测 RETEST-02）：以前返回 `0/0 张` 且 `ok:true`，
      // 看起来像「生图服务没输出」，实际只是字段名对不上（场景对象写的是 `id` 而不是 `scene_id`）。
      // 报错里带上「文件里实际有哪些 id」和「对象上实际有哪些字段」，一次调用就能定位。
      if (args.sceneId && wanted.length === 0) {
        const ids = scenes.map((s) => sceneIdOf(s)).filter((v) => v && v !== '?');
        const keys = scenes.length && scenes[0] && typeof scenes[0] === 'object'
          ? Object.keys(scenes[0]).slice(0, 12).join('、') : '（空）';
        throw new Error(`rp_scenes: 没有 scene_id="${args.sceneId}" 这一幕。`
          + `文件里实际有的 id：${ids.length ? ids.slice(0, 20).join('、') : '（一条都没有）'}`
          + `（按 scene_id / sceneId / id / title 识别）；第一条场景对象上的字段：${keys}。`
          + '约定字段是 `scene_id`，结构见工具说明里的最小示例。');
      }
      const panels = [];
      for (const scene of wanted) {
        for (const panel of (Array.isArray(scene?.panels) ? scene.panels : [])) panels.push({ scene, panel });
      }
      if (wanted.length && panels.length === 0) {
        throw new Error(`rp_scenes: 匹配到 ${wanted.length} 幕，但它们都没有 panels 数组（每格要有 positive/negative/seed/width/height）。`
          + '场景文件结构见工具说明里的最小示例。');
      }
      ...
      const errors = [];
      let done = 0;
      // 整幕多格共享一个 group：图墙里折叠成一叠，不然一套分镜会把面板刷满
      const group = `g${newAssetId()}`;
      const archived = [];
      for (const { scene, panel } of targets) {
        // 标签用**同一个**兜底函数：缺 `scene_id` 时退回 `id` / `sceneId` / `title`。
        // 以前只认 `scene_id`，缺了就存成「… · ?」进资源库 —— 那个问号会一直留在索引里，
        // 以后按名字根本认不出是哪一幕（真机复测的两张场景图就是这样）。
        const panelKey = panelIdOf(panel);
        const label = `${sceneIdOf(scene)}${panelKey ? `/${panelKey}` : ''}`;
        try {
          ...
          if (!w || !h) {
            ...
            const size = aspectToSize(aspect, 0.6);
          ...
          media.push(...result.media);
          // 每格都进资源库（同一个 group），这样整幕和单张都能被找回来
          const entries = await archiveGeneratedImages(cfg, sessionIdOf(exec), result, {
            kind: 'scene',
            label: [args.label, label].filter(Boolean).join(' · ').slice(0, ASSET_LABEL_MAX),
            tags: [String(scene?.title ?? '').trim(), String(panel?.panel_type ?? '').trim(), args.tags]
              .filter(Boolean).join(','),
            characters: charactersInPrompt(session, promptText),
            group,
          });
      ...
      return {
        ...
        media,
        errors,
        assets: archived.map((a) => a.id),
        group,
        note: `${st.label}｜${done}/${targets.length} 张｜来源 ${args.scenesFile}`
          + (archived.length ? `｜已存进资源库 ${archived.length} 张（同一组 ${group}）` : ''),
      };
    },
    /** 同 `rp_illustrate`：整幕批量本来就要跑很久，不该再被宿主的串行调度额外拖慢。 */
    isConcurrencySafe: () => true,
  },
  ...
  // ---- 资源库：浏览本会话已有的图，找到就复用，别重出 ----
  {
    name: 'rp_assets',
    description: [
      '浏览本会话的**图片资源库**：所有出过的图（角色立绘 / 场景 / 道具）和玩家导入的图都在里面，按分类分文件夹存着。',
      '为什么用它：本地出一张图要 15~25 秒且靠抽卡。**剧情回到同一个地方、同一个人再出场时，先把库里那张找出来重放**，',
      '比重新生成又快又稳（同一张图永远长得一样）。',
      '用法：',
      '- `action:"list"`（默认）按条件查：`kind` 选分类、`characters` 指定角色、`tags` 指定标签、`q` 关键词；',
      '- 返回的每一行都带 `id` 和**可直接放进 `dsh-ui` image 组件的地址**，选中哪张就把那个地址原样用上；',
      '- 出图时**填了 label/tags** 的图才搜得到（没填的只能靠 q 模糊匹配提示词原文）；',
      '- 事后发现某张图不好找，用 `action:"tag"` 补 label/tags；',
      '- 这个工具**不能删图**（删除是玩家在面板里做的事）。',
    ].join('\n'),
    parameters: {
      action: { type: 'string', description: '"list"（默认）查询；"get" 看单张详情（需 id）；"tag" 补 label/tags（需 id）' },
      kind: { type: 'string', description: 'list 用：分类过滤 —— portrait（角色）/ scene（场景）/ item（道具）/ other' },
      characters: { type: 'string', description: 'list 用：只看画面里有这些角色的图，逗号分隔（如「祁俊,祝婉宁」）' },
      tags: { type: 'string', description: 'list 用：按标签过滤，逗号分隔（要全部命中）' },
      ...
      label: { type: 'string', description: 'tag 用：改一个更好找的名字' },
    },
    output: {
      schema: {
        type: 'object',
    ...
    execute: async (args, exec) => {
      ...
      return {
        ...
        note: lines.length
          ...
          : `${head}；没有符合条件的图。需要就出新的，记得填 label 与 tags 方便以后找回来。`,
      };
    },
  },
 ...
 * 每个会话一把**写队列**。
 *
 * 为什么必须有：`rp_illustrate` 声明了并发安全（宿主并行池最多 10 个在飞），而索引是
 * read-modify-write —— 多个调用同时「读索引 → 加一条 → 写回」会互相覆盖，丢记录，而且
 * 丢了不报错（表现是「出了 5 张图，库里只有 2 张」）。所有写索引的路径都必须过这里。
 *
...
async function archiveAsset(sessionId, meta) {
  ...
  return withAssetLock(sessionId, () => {
    ...
    const dup = findDuplicateAsset(sessionId, index, kind, bytes, sha256);
    if (dup) {
      // 复用已有那条：补上它缺的标签、补上 hash，不写新文件
      let changed = false;
...
/** 从 ComfyUI `/view` 抓一张图的字节（归档要留一份自己的，不能只记引用）。 */
async function fetchComfyImageBytes(cfg, ref) {
  const base = String(cfg?.comfyui?.baseUrl || COMFY_BASE_URL).replace(/\/+$/, '');
  const qs = new URLSearchParams({
  ...
  if (!qs.get('filename')) return null;
...
/**
 * 出图之后把它收进资源库（`rp_illustrate` / `rp_scenes` / `rp_character` 的公共尾巴）。
 *
 * **不抛**：归档失败不该让一次成功的出图变成失败（模型会以为图没出来，可能重出）。
 */
async function archiveGeneratedImages(cfg, sessionId, result, opts = {}) {
  const files = Array.isArray(result?.files) ? result.files : [];
  if (!files.length) return [];
  const out = [];
  for (const ref of files) {
    try {
      const bytes = await fetchComfyImageBytes(cfg, ref);
      if (!bytes) continue;
      const entry = await archiveAsset(sessionId, {
        bytes, kind: opts.kind, ext: 'png',
        width: result.width, height: result.height,
        label: opts.label, tags: opts.tags,
        characters: opts.characters,
        style: result.styleKey, styleLabel: result.styleLabel,
        source: 'generated', group: opts.group, prompt: result.prompt ?? result.fullPrompt ?? '',
      });
...
/** 归一化一对宽高：只接受 256 以上的有限数字。 */
function normalizeSizePair(pair) {
...
/**
 * 取某个用途的尺寸：**全局 imageSizes 优先** → 风格的 sizes（手写特例）→ 兜底。
 *
 * 为什么全局优先：设置页现在只有一份「图像尺寸」，如果风格里还留着旧值，
 * 用户改了全局却发现出图没变（因为风格覆盖了）会非常困惑。要单风格特例就直接写 styles.json。
 */
function resolveImageSizes(cfg, style, sizeKey = 'scene') {
  const global = cfg?.imageSizes?.[sizeKey] ?? cfg?.imageSizes?.scene;
  const fromGlobal = normalizeSizePair(global);
  if (fromGlobal) return fromGlobal;
  const fromStyle = normalizeSizePair(style?.sizes?.[sizeKey] ?? style?.sizes?.scene);
  if (fromStyle) return fromStyle;
  return FALLBACK_IMAGE_SIZES[sizeKey] ?? FALLBACK_IMAGE_SIZES.scene;
}
 ...
 *
 * 早先还有一条「本机私有索引」的路（`lib/card-index.js`，从 `D:\Story\sillytavernassets`
 * 那个大卡库生成），但它与那个固定路径强绑定 —— 用户明确说「不需要判断那个目录」，
 * 于是整条索引路径都删掉了：卡库现在只由 `cards.root`（默认会话工作区下的 rp-cards）决定。
 */
let cardScanCache = new Map();  // root → entries

async function ensureCardIndex(root) {
  const rootAbs = resolve(root);
  const cached = cardScanCache.get(rootAbs);
  if (cached) return { entries: cached, source: 'scan' };
  return { entries: scanCardDir(rootAbs), source: 'scan' };
}

/** 扫目录：最多 3 层，跳过「解压密码」这类目录，只取文件名当卡名。 */
function scanCardDir(root, { depth = 3, limit = 8000 } = {}) {
  const out = [];
  const walk = (dir, level) => {
    ...
    for (const dirent of names) {
      ...
      if (dirent.isDirectory()) {
        ...
        const rel = full.slice(resolve(root).length + 1).split(sep).join('/');
        out.push({ f: rel, k: '', n: dirent.name.replace(/\.png$/i, ''), c: '', t: [], b: 0, s: size });
      }
    }
  };
  walk(resolve(root), 1);
  cardScanCache.set(root, out);
  return out;
}
...
/** 给设置页/工具用的风格摘要。`builtin=true` 的是内置风格（界面不给删）。 */
function styleSummary(cfg) {
  const builtin = new Set(Object.keys(defaultStyles().styles));
...
/**
 * 一个新风格的默认字段（「新增风格」表单提交时只用填 key/label/lora/trigger）。
 * 尺寸预设沿用内置默认，之后可以在卡片里改。
 */
function newStyleTemplate() {
  const anyBuiltin = Object.values(defaultStyles().styles)[0];
  return {
    workflow: anyBuiltin.workflow,
    lora: '',
    trigger: '',
    notes: '',
    cfg: anyBuiltin.cfg,
    steps: anyBuiltin.steps,
    sizes: JSON.parse(JSON.stringify(anyBuiltin.sizes)),
  };
}

/**
 * ComfyUI 里可用的 LoRA 清单（「选取本地 LoRA」用）。
 * 直接问 ComfyUI 的 /object_info —— 这是它的模型目录实况，不用自己扫盘猜路径。
 */
async function listLoras(cfg) {
  const res = await comfyApi(cfg, '/object_info/LoraLoaderModelOnly', { timeoutMs: 15000 });
  const spec = res.json?.LoraLoaderModelOnly?.input?.required?.lora_name;
  const options = Array.isArray(spec?.[0]) ? spec[0].map((n) => String(n)) : [];
  // 顺带标出是不是 krea2 风格 LoRA —— 本插件的 krea2 模板配别的 LoRA 多半出不了好图。
  return options.map((name) => ({
    name,
    krea2: /krea2/i.test(name),
  }));
}

/** 两段提交合并成一次状态变更：先看操作，再把字段补丁盖上去。 */
function applyStyleOps(cfg, ops) {
  const removed = [];
  const errors = [];
  // add/duplicate 失败的 key：调用方据此跳过字段补丁，避免盖到同名旧条目上
  const failedKeys = new Set();
  // 内置风格只允许改名/改字段，不允许删 —— 界面不给按钮，接口也必须独立挡住。
  const builtinKeys = new Set(Object.keys(defaultStyles().styles));
  for (const op of Array.isArray(ops) ? ops : []) {
    const action = String(op?.action ?? '');
    const key = String(op?.key ?? '').trim();
    try {
      if (action === 'remove') {
        if (!key) { errors.push('remove 缺少 key'); continue; }
        if (!cfg.styles?.[key]) { errors.push(`风格不存在：${key}`); continue; }
        if (builtinKeys.has(key)) { errors.push(`「${key}」是内置风格，不能删（可改 label / 触发词）`); continue; }
        delete cfg.styles[key];
        removed.push(key);
        if (cfg.defaultStyle === key) cfg.defaultStyle = Object.keys(cfg.styles)[0] ?? 'manga';
      } else if (action === 'add' || action === 'duplicate') {
        const from = String(op?.from ?? '').trim();
        if (!key) { errors.push('缺少风格 key'); failedKeys.add(key); continue; }
        if (!/^[\w.-]+$/.test(key)) { errors.push(`key 只能用字母/数字/下划线/点/横线：${key}`); failedKeys.add(key); continue; }
        if (cfg.styles?.[key]) { errors.push(`风格已存在：${key}`); failedKeys.add(key); continue; }
        if (action === 'duplicate') {
          if (!cfg.styles?.[from]) { errors.push(`要复制的风格不存在：${from}`); failedKeys.add(key); continue; }
          cfg.styles[key] = JSON.parse(JSON.stringify({ ...cfg.styles[from], label: String(op?.label ?? `${cfg.styles[from].label} 副本`) }));
        } else {
          cfg.styles[key] = { ...newStyleTemplate(), label: String(op?.label ?? key) };
        }
      } else {
        errors.push(`未知操作：${action}`);
      }
    } catch (error) {
      errors.push(String(error?.message ?? error));
    }
  }
  return { removed, errors, failedKeys };
}
...
/**
 * 「本会话设定」块：DM 自己的规则 + 生图开关 + **已有立绘可直接用的图片地址**。
 *
 * 为什么要放进常驻段：
 * - DM 设定（多半来自 DM 卡的导入）就是**这个 DM 的行为准则**，属于指令，必须在系统提示里；
 * - 生图开关决定它要不要调 `rp_illustrate`，只有宿主知道；
 * - 「谁已经有立绘」这件事由宿主算最准 —— DM 拿到现成地址就能**直接展示**，不必重新生成
...
function renderDmSetup(session, { loreFile } = {}) {
  ...
  const img = dm.images ?? {};
  const lines = [];
  // 生图开关：DM 只认这一行（本地是否真的能出图，它调一次 rp_styles 就知道）
  if (img.enabled === false) {
    lines.push('- 生图：**关**（本会话不配图；不要调 rp_illustrate。玩家明确要求时先问一句再出）');
  } else {
    const when = [
      img.firstAppearance !== false ? '角色第一次出场给 1 张立绘' : '',
      img.keyScenes !== false ? '重要场景给 1 张氛围图' : '',
    ].filter(Boolean).join('；') || '仅在玩家要求时';
    lines.push(`- 生图：开（${when}）· 出图用 \`rp_illustrate\`，拿到返回的 media[0] 后用 \`dsh-ui\` 的 image 组件把它显示出来；过场小动作不要出图`
      + `；**要 2 张以上时在同一步里并发发出多个 \`rp_illustrate\`**（一张 15~25 秒，串行等于把等待叠加）`);
  }
  ...
  for (const [name, entry] of Object.entries(portraits)) {
    ...
    const gen = entry.generated;
    if (gen && typeof gen === 'object' && gen.file) {
      arts.push(`  · ${name}：/rp-tools/media?file=${encodeURIComponent(String(gen.file))}&subfolder=${encodeURIComponent(String(gen.subfolder ?? ''))}&type=${encodeURIComponent(String(gen.type ?? 'output'))}&sessionId=${encodeURIComponent(String(session?.sessionId ?? ''))}`);
    } else if (entry.imported?.file) {
      // 用户外部导入的立绘（存在会话目录里，走 /rp-tools/portrait-image）。
  ...
  // 面板也摆在【世界设定】旁边，第一次出场时**直接展示它**，不必再花 15~25 秒出一张。
  ...
  if (coverCard) {
    ...
    // 已经有真立绘（生成的或外部导入的）或已登记过卡面 → 不重复列，免得 DM 以为有两张图
    const ownerHasArt = Boolean(ownerArt && (ownerArt.card || ownerArt.generated?.file || ownerArt.imported?.file));
  ...
  if (arts.length) {
    lines.push('- **已有可用图：先在下面找，有就直接展示，不要再生成**（生成的立绘 / 玩家导入的图 / 导入卡的卡面都在这里；展示时把地址原样放进 `dsh-ui` 的 image 组件）：');
    lines.push(...arts);
  }
  ...
  if (assetTotal > 0) {
    ...
    lines.push(`- 资源库：本会话已有 ${assetTotal} 张图（${parts.join('、')}）—— 剧情回到同一地点/同一人物时，`
      + '**先用 `rp_assets` 按标签或角色查一遍，找到就重放那张**（同一张图永远长得一样，还省 15~25 秒）；确实没有才出新的');
  } else {
    // 同上：空的时候明说，别让「这一行没出现」被读成「已经有了」
    lines.push('- 资源库：本会话**还没有任何图**（一张都没有，需要就出新的；出图时记得填 `label` 与 `tags`）');
  }
...
function buildStandingText(session, extra = {}) {
  if (!session) return '';
  const parts = [];
  // 「有没有世界观类内容」决定用哪个标题：只有 DM 设定（生图开关等）时不该声称「设定都已确立」
...
function installRoutes(ctx) {
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          // 卡库目录 + 默认宏列表（PNG 故事书导入用）
          if (body.cards && typeof body.cards === 'object') {
            const incoming = body.cards.macros;
            const macros = {};
            if (incoming && typeof incoming === 'object') {
              // 只收合法名字 + 非空值（界面上还没填完的行不该变成一条空默认值）
            ...
            cfg.cards = {
              root: String(body.cards.root ?? '').trim(),
              // userLabel 继续跟着 user 走：它只是老字段，别让它和宏列表说两套话
              userLabel: String(macros.user ?? ''),
              macros,
              // 保存过就按用户自己的来：之后不再自动补 `user -> 玩家`
              // （否则他删掉那一条，下次打开又冒出来）
              macrosSeeded: true,
            };
          }
          // 全局图像尺寸（设置页「图像」里那三行）：只接受那三个槽位 + 合法宽高
          if (body.imageSizes && typeof body.imageSizes === 'object') {
            const next = { ...(cfg.imageSizes ?? {}) };
            for (const slot of IMAGE_SIZE_SLOTS) {
              const pair = normalizeSizePair(body.imageSizes[slot]);
              if (pair) next[slot] = pair;
            }
            cfg.imageSizes = next;
          }
          if (body.campaign && typeof body.campaign === 'object') {
            // 全局 campaign 只有角色卡表（负面词是 cfg.negative，不属于 campaign）
            cfg.campaign = {
          ...
          // 风格操作必须在字段补丁**之前**：新增操作先把条目建出来，后面的补丁才盖得上去。
          // （顺序反了的话 `if (!st) continue` 会把新风格的全部字段静默丢掉。）
          const opsResult = applyStyleOps(cfg, body.styleOps);
          if (body.styles && typeof body.styles === 'object') {
            for (const [key, patch] of Object.entries(body.styles)) {
              // add/duplicate 失败的 key 不许落补丁 —— 否则会盖到旧条目上，把已有风格静默改掉。
              if (opsResult.failedKeys.has(key)) continue;
              ...
              // lora：空串 = 不使用 LoRA（走提示词风格）
              if (typeof patch.lora === 'string') st.lora = patch.lora.trim();
              if (typeof patch.workflow === 'string' && patch.workflow) st.workflow = patch.workflow;
          ...
          sendJson(response, 200, {
            ...
            config: cfg,
            styles: styleSummary(cfg),
            removedStyles: opsResult.removed,
            styleErrors: opsResult.errors,
          });
        } catch (error) {
    ...
    // 本地可选 LoRA（「新增风格」里挑 LoRA 用）——直接读 ComfyUI 的模型目录实况
    hostCtx.effect(() => hostCtx.webServer.register({
      kind: 'exact',
      path: '/rp-tools/loras',
      handler: async (request, response) => {
        ...
        try {
          const cfg = loadStyles();
          const loras = await listLoras(cfg);
          sendJson(response, 200, { ok: true, loras, count: loras.length });
        } catch (error) {
          // ComfyUI 没开也要能用设置页：返回结构化失败，界面回退成手填
          sendJson(response, 200, { ok: false, loras: [], error: String(error?.message ?? error) });
        }
      },
    }), 'rp-tools: loras');
    ...
    // 会话级配置：读/写某个会话的 RP 设置（角色卡 / 世界 / 前缀 / 风格备注 / 战役名 / 会话默认风格 / 随机表）
    hostCtx.effect(() => hostCtx.webServer.register({
      kind: 'exact',
      ...
      handler: async (request, response) => {
        ...
        if (request.method === 'POST') {
          ...
          try {
            ...
            if (body.campaign && typeof body.campaign === 'object') {
              // 会话 campaign 只有战役名 + 提示词前缀；负面词是全局的，不收
              session.campaign = {
                name: String(body.campaign.name ?? ''),
                prompt_prefix: String(body.campaign.prompt_prefix ?? ''),
              };
            ...
            // DM 设定（本会话）：DM 自己的规则 + 生图开关（会话隔离）。
            // 面板提交的是完整对象；只收认识的键，非法值不动旧值。
            if (body.dm && typeof body.dm === 'object') {
              const img = body.dm.images && typeof body.dm.images === 'object' ? body.dm.images : null;
              session.dm = {
                prompt: typeof body.dm.prompt === 'string' ? body.dm.prompt : session.dm?.prompt ?? '',
                migrated: [],                                  // 用户保存过就清掉「迁移提示」
                images: {
                  enabled: typeof img?.enabled === 'boolean' ? img.enabled : session.dm?.images?.enabled !== false,
                  firstAppearance: typeof img?.firstAppearance === 'boolean' ? img.firstAppearance : session.dm?.images?.firstAppearance !== false,
                  keyScenes: typeof img?.keyScenes === 'boolean' ? img.keyScenes : session.dm?.images?.keyScenes !== false,
                },
              };
            }
    ...
    // ── 立绘：把生成结果记进**会话配置**，下次打开面板还能看到 ────────────
    // 为什么必须有这条：立绘原先只存在面板组件的 state 里，一关面板/刷新就没了 ——
    // 用户报的「角色卡生成的立绘下次打开就消失了」就是这个。
    // 只存 ComfyUI 的 (file, subfolder, type) 三要素（不存 URL：origin 可能变），
    // 媒体仍走 /rp-tools/media 代理；卡面（导入时的 card 字段）保持不动，两者并存。
    hostCtx.effect(() => hostCtx.webServer.register({
      kind: 'exact',
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          if (String(body.action ?? 'save') === 'clear') {
            // 只清掉生成的那张；卡面（导入卡时登记的 card）留着，它是另一份东西
            ...
            if (!file) { sendJson(response, 400, { ok: false, error: 'file is required' }); return; }
            portraits[name] = {
              ...(portraits[name] ?? {}),
              generated: {
                file,
          ...
          // 面板出的立绘也要进资源库（真存一份字节）：这样图墙里能看到它、能提回来当别人的立绘、
          // 也能被 DM 搜索复用。抓不到字节不算失败 —— 立绘本身已经记进会话配置了。
          if (saved && String(body.action ?? 'save') !== 'clear') {
            const cfgNow = loadStyles();
            await archiveGeneratedImages(cfgNow, id, {
              files: [{ file: String(body.file ?? ''), subfolder: String(body.subfolder ?? ''), type: String(body.type ?? 'output') }],
              styleKey: String(body.style ?? ''), styleLabel: String(body.style ?? ''),
              width: Number(body.width) || 0, height: Number(body.height) || 0,
              prompt: `立绘：${name}`,
            }, { kind: 'portrait', label: name, characters: [name] });
          }
          ...
          const result = await withAssetLock(id, () => {
            ...
            if (action === 'delete') {
              ...
              try {
                ...
                if (abs.startsWith(p.dir + sep) && existsSync(abs)) { rmSync(abs, { force: true }); fileGone = true; }
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          const result = await generateOne(cfg, {
            ...
            // 尺寸档：面板出**立绘**时传 `portrait`（纵向），试出/场景图不传就是 scene
            sizeKey: IMAGE_SIZE_SLOTS.includes(String(body.sizeKey ?? '')) ? String(body.sizeKey) : 'scene',
            timeoutMs: 240000,
            session,
          });
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: (request, response) => {
        ...
        try {
          ...
          if (!got.ok) { sendJson(response, 400, { ok: false, error: got.error, warnings: got.warnings ?? [] }); return; }
        ...
        try {
          ...
          if (got.built.isDmCard === true && got.built.dmPrompt) {
            session.dm = {
              ...
              migrated: [],
              images: session.dm?.images ?? { enabled: true, firstAppearance: true, keyScenes: true },
            };
          }
          if (isCharacter) {
...
export const __debug = {
  ...
  normalizeLoreKey,
  loreNameConflicts,
  recordGeneratedPortrait,
  illustrateSizeKey,
  // 资源库（测试要能直接验归档/索引/并发写，不必真跑 ComfyUI）
  loadAssets,
  saveAssets,
  archiveAsset,
  archiveGeneratedImages,
  fetchComfyImageBytes,
  findDuplicateAsset,
  listLaunchFiles,
  sceneIdOf,
  panelIdOf,
  ...
  resolveImageSizes: (cfg, styleKey, slot) => resolveImageSizes(cfg, cfg?.styles?.[styleKey], slot),
