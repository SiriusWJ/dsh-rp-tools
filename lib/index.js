...
function defaultStyles() {
  ...
  return {
    ...
    cards: { root: '', userLabel: '' },   // 卡库根目录 + 导入时的玩家称呼（'' = 用默认「玩家」）
    styles: {
...
function loadStyles() {
  ...
  // 老配置迁移：卡库目录 + 玩家称呼（PNG 故事书导入用）
  if (!cfg.cards || typeof cfg.cards !== 'object') {
    cfg.cards = { root: '', userLabel: '' };
  } else if (typeof cfg.cards.userLabel !== 'string') {
    cfg.cards = { ...cfg.cards, userLabel: '' };
  }
  // 老配置迁移：风格库只留「二次元 / 写实 / 黑白漫画」，其余内置风格清掉
  {
    ...
    const DEFAULTS = defaultStyles().styles;
    for (const [key, st] of Object.entries(cfg.styles ?? {})) {
...
function sessionLorePath(sessionId, hint) {
  ...
  // 工作区优先取宿主记录的（session/created 里的 header.cwd），没有就用调用方给的绝对路径。
  // 路由侧总是能拿到客户端传来的工作区，而装配热路径只有 sessionCwd —— 两者是同一个目录。
  let cwd = sessionCwd.get(id);
  if (!cwd) {
    const raw = String(hint ?? '').trim();
    if (raw && isAbsolute(raw) && existsSync(raw)) {
      cwd = resolve(raw);
      sessionCwd.set(id, cwd);      // 顺手补登记，让装配热路径也找得到同一个文件
    }
  }
  if (!cwd) return null;
  const dir = join(cwd, LORE_SESSION_DIR, id);
  return { id, cwd, dir, file: join(dir, LORE_FILE_NAME), legacy: join(cwd, LORE_FILE_NAME) };
}
    ...
    execute: async (args, exec) => {
      ...
      if (action === 'set') {
        ...
        if (args.macro_name !== undefined) {
          ...
          macroValueCache.set(sessionId, { user: cardUserLabel(loadStyles()), ...session.macros });
...
function cardUserLabel(cfg) {
  const label = String(cfg?.cards?.userLabel ?? '').trim();
...
function readCard(root, rel, { userLabel } = {}) {
  ...
  if (!abs) return { ok: false, error: '路径不合法（必须在卡库目录内的 .png）' };
  let size = 0;
  try {
  ...
  const built = buildImport(decoded, { userLabel });
...
function writeImportFiles(workspace, { root, rel, built, decoded, name, userLabel, chosenGreeting, sessionId }) {
  ...
  const md = cardToMarkdown(decoded, { maxChars: CARD_MD_MAX_CHARS, userLabel });
...
function installStandingPrompt(ctx, opts = {}) {
  try {
    ...
    const providerFor = (name) => () => {
      const table = macroValueCache.get(selfSessionId) ?? {};
      if (name === 'user') {
    ...
    macroValueCache.set(selfSessionId, { ...initial });
    registerVar('user');
    ...
    const off = ctx.on('system-prompt/assemble', async (assembly, context, next) => {
      ...
      try {
        ...
        // 值缓存先刷新（provider 读它），再把宏表里新出现的名字补注册 ——
        // 用户刚在面板里加一个 `{{place}}`，下一轮装配就能用上。
        macroValueCache.set(sessionId, { user: cardUserLabel(loadStyles()), ...(session.macros ?? {}) });
        refreshSessionMacros(sessionId, session.macros ?? {});
  ...
  ctx.inject(['webServer'], (hostCtx) => {
        ...
        if (request.method === 'POST') {
          ...
          try {
            ...
            if (body.macros && typeof body.macros === 'object') {
              ...
              macroValueCache.set(normalizeSessionId(id), { user: cardUserLabel(loadStyles()), ...session.macros });
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          macroValueCache.set(normalizeSessionId(id), { user: cardUserLabel(cfg), ...(session.macros ?? {}) });
          refreshSessionMacros(id, session.macros ?? {});
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: (request, response) => {
        ...
        try {
          const url = new URL(request.url ?? '/', 'http://localhost');
          const abs = safeCardPath(cardLibraryRoot(loadStyles(), resolveWorkspaceDir(url.searchParams.get('sessionId') ?? '', url.searchParams.get('workspace'))), url.searchParams.get('path') ?? '');
...
export const __debug = {
  ...
  AUTO_MACROS,
  autoMacroValue,
  /** 测试用：给某个会话登记工作区目录（生产由 session/created 填充）。 */
  ...
  try {
    ctx.on('session/created', (session) => {
      try {
        ...
        if (typeof cwd === 'string' && cwd) sessionCwd.set(id, cwd);
