...
function defaultStyles() {
  ...
  return {
    ...
    // PNG 故事书导入的卡库根目录（空串 = 用 CARD_DEFAULT_ROOT）
    cards: { root: '', userLabel: '' },   // 卡库根目录 + 导入时的玩家称呼（'' = 用默认「玩家」）
    styles: {
      anime: mk('二次元', null, 'anime style, japanese animation key visual, clean crisp lineart, cel shading, vibrant colors, detailed expressive eyes, soft rim light, character focus', '通用二次元动画风（无 LoRA，靠提示词；想更浓的赛璐璐味可在本风格里挂 krea2_retroanime）'),
      realistic: mk('写实', null, 'photorealistic, ultra detailed skin texture, natural soft lighting, 50mm lens, shallow depth of field, subtle film grain, cinematic realism', '写实照片风（无 LoRA，靠提示词；人像/场景通用）'),
 ...
 * 老配置里这些键要清掉（它们是我们的默认值，不是用户自己造的风格）；
 * `defaultStyle` 若指向被删的键，回落到 `manga`。
 */
...
function loadStyles() {
  ...
  // 老配置迁移：风格库只留「二次元 / 写实 / 黑白漫画」，其余内置风格清掉
  {
    ...
    // 默认风格若指向被删的键 → 回落到 manga（否则生图会因为找不到风格报错）
    if (!cfg.styles?.[cfg.defaultStyle]) {
      cfg.defaultStyle = cfg.styles?.manga ? 'manga' : (Object.keys(cfg.styles ?? {})[0] ?? 'manga');
      dropped = true;
    }
...
/** 默认卡库根目录（可在 styles.json 的 `cards.root` 覆盖）。 */
const CARD_DEFAULT_ROOT = 'D:\\Story\\sillytavernassets';
 ...
 * 3. 拿不到工作区时才回落到内置的那个大卡库（`CARD_DEFAULT_ROOT`）。
 */
 ...
 * 卡索引。
 *
 * `lib/card-index.js` 是**本机私有的**可选文件（已 gitignore，含 3269 张卡的
 * 卡名/作者/标签/世界书条数），所以用**动态 import + try** 拿它 —— 静态 import
 * 一旦文件不存在会让整个插件加载失败（这条踩过一次：删掉索引后 400 满屏）。
 * 没有索引时退回**扫目录**（只认文件名，扫不到卡名/标签），功能仍然可用。
 */
let cardIndexCache = null;      // null = 还没试过；[] = 试过但没有
let cardScanCache = new Map();  // root → entries

async function ensureCardIndex(root) {
  const rootAbs = resolve(root);
  // 私有索引里的相对路径是**相对默认卡库根**的，换了根目录它就完全不适用 ——
  // 拿它去列别的目录会列出一堆不存在的路径（踩过一次：临时库列出 3269 张真卡）。
  const isDefaultRoot = rootAbs.toLowerCase() === resolve(CARD_DEFAULT_ROOT).toLowerCase();
  if (isDefaultRoot) {
    if (cardIndexCache === null) {
      try {
        const mod = await import('./card-index.js');
        cardIndexCache = Array.isArray(mod?.CARD_INDEX) ? mod.CARD_INDEX : [];
      } catch { cardIndexCache = []; }
    }
    if (cardIndexCache.length) return { entries: cardIndexCache, source: 'index' };
  }
...
/** 索引缺失时的兜底：按目录扫 PNG（最多 3 层，只取文件名当卡名）。 */
