 ...
 */
export function buildOpeningPrompt(imported, { worldFile, cardFile, imageRel, greeting } = {}) {
  ...
  const cut = raw.length > OPENING_GREETING_CHARS ? clamp(raw, OPENING_GREETING_CHARS) : raw;
  const lines = [
  ...
  if (cut) {
    lines.push(
      ...
      + `${cut.length < raw.length ? `（原文较长，这里给的是节选，全文见上面的卡组全文文件）` : ''}`,
      '',
      cut,
    );
  }
