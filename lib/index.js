...
/** 卡库根目录（配置优先）。 */
function cardLibraryRoot(cfg) {
  const fromCfg = String(cfg?.cards?.root ?? '').trim();
  return fromCfg || CARD_DEFAULT_ROOT;
}
  ...
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          const url = new URL(request.url ?? '/', 'http://localhost');
          const cfg = loadStyles();
          const root = cardLibraryRoot(cfg);
          const { entries, source } = await ensureCardIndex(root);
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: (request, response) => {
        ...
        try {
          const url = new URL(request.url ?? '/', 'http://localhost');
          const abs = safeCardPath(cardLibraryRoot(loadStyles()), url.searchParams.get('path') ?? '');
