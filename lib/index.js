...
import { buildImport, cardToMarkdown, buildOpeningPrompt } from './card-import.js';
...
const WORKFLOW_TEMPLATES = {
  ...
  uncensored_anime: (st) => {
    const lora = st.lora ? String(st.lora) : '';
    const nodes = {
      '1': { class_type: 'CheckpointLoaderSimple', inputs: { checkpoint_name: 'NetaYumev35_pretrained_all_in_one.safetensors' } },
    ...
    const nodes = {
      ...
      '6': {
        ...
        inputs: {
          ...
          sampler_name: 'euler', scheduler: 'simple',
          positive: ['4', 0], negative: null, latent_image: ['5', 0], denoise: 1,
        },
      },
...
function writeImportFiles(workspace, { root, rel, built, decoded, name }) {
  ...
  const md = cardToMarkdown(decoded, { maxChars: CARD_MD_MAX_CHARS });
  ...
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          // 卡库目录：PNG 故事书导入功能的扫描根（空串 = 回到内置默认）
          if (body.cards && typeof body.cards === 'object') {
            cfg.cards = { root: String(body.cards.root ?? '').trim() };
          }
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: (request, response) => {
        ...
        try {
          const url = new URL(request.url ?? '/', 'http://localhost');
          const root = cardLibraryRoot(loadStyles());
          const rel = url.searchParams.get('path') ?? '';
          const got = readCard(root, rel);
          if (!got.ok) { sendJson(response, 400, { ok: false, error: got.error, warnings: got.warnings ?? [] }); return; }
          const { character, world, stats, summary } = got.built;
          sendJson(response, 200, {
            ok: true,
            path: rel,
            name: character.name,
