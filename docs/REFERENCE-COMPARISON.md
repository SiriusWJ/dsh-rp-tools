...
>
> **⚠️ v1.15.0：本插件的本地生图（ComfyUI）已整条移除。** 本文主体写于 v1.1.2，§2 / §3 / §7 里
> 「直连 ComfyUI 出图 + 风格 LoRA + 同源媒体代理」**已不是我们的能力**（出图改由宿主的 `generate_image` / `edit_image` 承担）；
> 差异化落在 **会话隔离 + 图片资源库复用 + 世界书/状态 + 骰子/随机表**。
> 描述「当前能力」的段落已按新现实改写；**保留原样的段落一律标了「作废」或「历史」**。
...
我们在**工程层**（按会话隔离、宿主集成、图片资源库、fork 继承）上并不落后，~~甚至在「直连 ComfyUI 出图」这一点上是独有的~~ **（该差异化已于 1.15.0 作废：插件不再出图，出图交给宿主的 `generate_image` / `edit_image`）**。
...
## 2. 我们的现状（dsh-rp-tools v1.15.0）

**已具备**：
- 8 个工具：`rp_random`（骰子/区间/加权/布尔，seed 可复现）、`rp_character`、`rp_state`、
  `rp_lore`、`rp_session`、`rp_assets`、`rp_config`、`rp_table`
  （~~`rp_styles` / `rp_illustrate` / `rp_scenes`~~ **1.15.0 移除**）
- **图片的入口与复用**：`rp_assets` 的 `list` / `get` / `tag` / **`import`** —— 出图由宿主的
  `generate_image` / `edit_image` 负责，插件把图收进**本会话资源库**，再按 kind / 角色 / 标签 / 关键词查出来重放
- ~~**直连 ComfyUI** 的 API 工作流（krea2 turbo + 风格 LoRA）+ 自建同源媒体代理~~ **（1.15.0 移除）**
- ~~10 种内置风格 + 自定义风格增删 + **本地 LoRA 下拉选取**~~ **（1.15.0 移除，风格改由提示词承担）**
- 按会话隔离：世界设定 / 角色卡 / 资源库 / 随机表 / 提示词前缀 / 战役名 / DM 设定 / 宏
  （~~风格备注 / 会话默认风格~~ **1.15.0 移除**）
- fork 时按 `header.parentSession + isSeeded` 继承会话配置
- 界面：设置页（卡库目录 + 默认宏列表 + 工具列表）、会话头部右上角入口、**右侧栏页签面板**（DM 设定 / 世界设定 / 宏变量 / 世界书 / 角色卡 / **资源库** / 备份·会话包 / 随机表 / 当前状态，每个角色带立绘位）
- 冒烟测试：`smoke-dm` 1004 条 + `smoke-client` 427 条 + `smoke-card` 256 条 + `verify-roundtrip` 10 条，**0 失败**
...
| **角色卡字段深度** | ST V2/V3 规范（`first_mes`/`mes_example`/`system_prompt`…）；hermes 标明 `first_mes` 最重要 | ~~只有 `name` + `appearance`（且只服务于生图）~~ 已补齐为 8 字段（见 §7.2）；`appearance` 现在服务于**画面描述与注入**，不再是插件自己的生图输入 |
...
2. **角色卡字段加深 + `first_mes` / `mes_example`**
   ...
   注意：~~我们的角色卡现在只服务生图，要扩成「既服务生图、也服务叙事」。~~ 已完成：8 字段、既服务叙事也服务画面描述（1.15.0 起插件不再出图，`appearance` 只作为 `generate_image` 的画面描述素材）。
3. **我的人设（persona）**
...
- ~~**直连 ComfyUI 的生图链路**~~ **（1.15.0 作废 —— 插件不再出图，出图是宿主的 `generate_image` / `edit_image`）**：两个参考项目都不做本地出图（liketavern 有消息图片渲染，但不是生成）
- **图片资源库复用**：本会话的图（宿主生成的 + 玩家导入的）收进 `rp_assets`，同一角色/场景再出场时**先把库里那张重放出来**，不再重复生成
- **立绘/场景图与角色卡绑定**：角色卡 → 外观锚点 → 立绘位（立绘只来自**玩家导入的图**或导入卡的卡面），这条线是我们的差异化
...
2. **`filterUnavailableToolPromptSections`**：把「工具说明段」与**当前真正可见的工具**对齐 ——
   ...
   ~~但**提示词里仍可能残留「你有 rp_illustrate」这类指导**~~ —— **这类问题已修（1.15.0）**：
   persona 不再点名任何已删工具（点名也没用，模型本来就看不到它们），而反向的坑更值得记 ——
   宿主的 `generate_image` / `edit_image` 是**全局**工具，必须在 dm-filter 的 `keepGlobalTools` 里放行，
   否则 persona 里「用 `generate_image` 配图」就是一句空话。**说明段与实际可见工具，两边都要对齐。**
...
我们的差异化是**结构性的**：~~直连 ComfyUI 的生图链路 + 风格 LoRA + 同源媒体代理~~ **（1.15.0 作废 —— 插件不再出图）**，
现在的落点是 **会话隔离 + 图片资源库复用 + 世界书/状态追踪 + 骰子/随机表**。
它们仍然有而我们**没有**的是：角色卡生态兼容（ST/PNG 卡格式）、长期记忆/摘要、完整消息操作。

所以定位是：**轻量跑团配图 + 裁决工具** —— 出图交给宿主，我们负责**把图管好、把设定与状态记住**，而不是变成第三个叙事引擎。
...
| 2 | **角色卡字段加深（学 hermes，不导入 ST 格式）** | ~~现在只有 `name`+`appearance`，且只为生图服务~~ 已补齐为 8 字段；`appearance` 现在服务于**画面描述与注入**，不再是插件自己的生图输入 | 小 | ✅ 已完成 |
| 3 | **世界书：关键词触发检索** | 把「设定体量」与「每轮注入量」解耦；长会话里固定注入必然吃光上下文 | 中 | ✅ 已完成 |
| 4 | ~~**结构化 `appearance` → 生图一致 prompt**~~ | ~~我们唯一的护城河就是生图，而角色一致性是它最大的痛点~~ **（1.15.0 作废：插件不再出图，`characterSeed()` 一类机制已删）**。现在只剩一条纪律 —— 把 `appearance` 写进 `generate_image` 的画面描述 | 中 | ❌ 已作废 |

**#2 已完成的字段集**（`CHARACTER_FIELDS`，故意**不做** ST/CCv 兼容与 PNG 导入 —— 定位是轻量工具）：

| 字段 | 取向 |
|---|---|
| `name` | 唯一标识 + 注入检索键 + 配图时的角色锚点 |
| `appearance` | **画面描述素材**（配图时进 `generate_image` 的 prompt）+ 注入。填写**顺序建议**写进工具描述（发型色→眼睛→肤色体型→身高→穿着→配饰） |
...
| # | 做什么 | 为什么 | 难度 |
|---|---|---|---|
| 5 | **Writer / Commit 分离** | 结构上杜绝「正文与副作用混杂」。~~**我们的生图工具最受益**：现在生图和剧情挤在同一条消息里，很容易图文不一致~~ **（1.15.0 起插件没有生图工具了，这条动机减弱）** | 中 |
...
| 8 | **工具说明段与实际可见工具对齐** | 我们给 dm 预设过滤了工具，提示词里曾残留「你有 rp_illustrate」这类指导。**典型的「说明段与实际可见工具不一致」bug**（1.15.0 已修：persona 不再点名任何已删工具；反向的坑是宿主的 `generate_image` / `edit_image` 属全局工具，必须写进 dm-filter 的 `keepGlobalTools` 才看得见） | 小 |
...
| ~~角色生图一致性（第一版）~~ | ❌ **（1.15.0 移除本地生图时一并删除）** | ~~`characterSeed()`：同角色固定 seed；多角色同框时不固定~~ 现在只剩一条纪律：把 `appearance` 写进 `generate_image` 的画面描述 |
| **状态追踪** | ✅ | `rp_state` + `applyStateUpdates` / `renderState`，注入在 turn 通道最前 |
| 按条补读、禁止整本倾倒 | ✅ | `rp_lore` 的 `list` / `find` / `template` 三个动作 |
| **图片资源库（入库 + 复用）** | ✅ | `rp_assets` 的 `list` / `get` / `tag` / `import`（出图由宿主的 `generate_image` / `edit_image` 负责） |
| ~~本地 LoRA 选取~~ | ❌ **（1.15.0 移除）** | ~~`/rp-tools/loras` + 设置页下拉~~ —— 「风格」改由 prompt 里的风格词承担 |
...
|---|---|---|---|---|
| A | **角色一致性（同一角色跨场景长得一样）** | 三家都没有真做（hermes 只给了外观字段顺序纪律） | ~~我们唯一的护城河就是生图，而这是它最大的痛点~~ **（1.15.0 作废：插件不再出图，固定种子机制已删）**；现在的做法只剩「把 `appearance` 写进画面描述」 | 小（纪律） |
| B | ~~**角色卡按需注入**~~ | 关键词激活 | ~~现在全量注入，人一多就膨胀；数据已就绪~~ **已补上，见 §8.1**（`renderCharacterIndex` 常驻只放索引 + `charactersToExpand` 出场才展开） | 小 |
| C | ~~**状态追踪（关系/物品/地点/伤势）**~~ | roleplay 的 `rp-state`：event-sourced 投影 + 双面契约 | ~~完全没有~~ **已补上，见 §8.1**（`rp_state`；投影/双面契约那套刻意没抄，用工具 + 渲染代替） | 中大 |
| D | **输出的机读状态块 + 正则收口** | hermes 状态栏 + liketavern `output/render` 正则 | 完全没有；且这是 A/B 的天然触发器（场景一变就该配图） | 中 |
| E | **Writer/Commit 分离** | roleplay 把叙事与副作用提交分开 | ~~生图和剧情挤在同一条消息里，容易图文不一致~~ 动机减弱：1.15.0 起出图由宿主的 `generate_image` 承担，插件不再自己出图；但「配图 + `rp_assets(import)` + 状态更新」仍与叙事混在同一条消息里 | 中 |
...
### 8.4 ~~A 的可行性已实测~~ **A 的可行性（1.15.0 作废 —— 插件已无生图）**

> ⚠️ **整节作废。** 下面是「接 Qwen-Image-Edit 做角色一致性」的环境调查记录，但 1.15.0 已把本地生图链路
> 整条移除（工作流模板、`characterSeed()`、参考图路线全部不在仓库里了），所以**这些结论不再对应任何待做项**，
> 只作历史环境记录保留 —— 别照着它去实现。

~~本机 ComfyUI 环境（用 `/object_info` 查的实况，不是猜的）：~~ **当时本机的 ComfyUI 环境（历史记录）：**
...
参考图作为第二步。~~ **（两版都没做，现在也不必做了 —— 1.15.0 起插件不再出图。）**
