...
import { buildImport, cardToMarkdown, buildOpeningPrompt, DEFAULT_USER_LABEL, localizeAttributes } from './card-import.js';
...
function writeImportFiles(workspace, { root, rel, built, decoded, name, userLabel }) {
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          const opening = buildOpeningPrompt(got.built, {
            ...
            greeting: got.built.greeting,     // 未截断的原文（角色字段里那份是 1200 字短版）
          });
          sendJson(response, 200, {
            ok: true,
            sessionId: id,
