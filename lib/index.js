...
import { buildImport, cardToMarkdown, cardToOpeningFile, buildOpeningPrompt, DEFAULT_USER_LABEL, localizeAttributes, listGreetings, discoverMacros, collectCardText, MACRO_NAME_RE, AUTO_MACROS, autoMacroValue } from './card-import.js';
...
const DEFAULT_IMAGE_SIZES = { scene: [1024, 576], portrait: [640, 896], item: [768, 768] };
...
function defaultStyles() {
  const mk = (label, lora, trigger, notes, extra = {}) => ({
    ...
    cfg: 1,
    steps: 8,
    sizes: { scene: [1024, 576], portrait: [640, 896], item: [768, 768] },
    ...extra,
  });
  return {
    ...
    styles: {
      ...
      uncensored_anime: mk('二次元', 'tone001_manga.safetensors',
        ...
        { workflow: 'uncensored_anime', cfg: 5.5, steps: 20, sizes: { scene: [1024, 576], portrait: [640, 896], item: [768, 768] } }),
      uncensored_real: mk('写实', null,
        ...
        { workflow: 'uncensored_real', cfg: 1, steps: 20, sizes: { scene: [1024, 576], portrait: [640, 896], item: [768, 768] } }),
      manga: mk('黑白漫画', null,
...
const LEGACY_STYLE_SIZES = [[1344, 768], [1024, 1024], [768, 1024]];
...
function loadStyles() {
  ...
  // 老配置迁移：**全局图像尺寸**（1.9.1 起尺寸从风格里提到全局统一编辑）
  {
    ...
    for (const slot of IMAGE_SIZE_SLOTS) {
      const pair = normalizeSizePair(src[slot]) ?? DEFAULT_IMAGE_SIZES[slot];
      out[slot] = pair;
      if (normalizeSizePair(src[slot]) === null) fixed = true;
    }
...
function loadSession(sessionId) {
  ...
  return {
    ...
    characters: Array.isArray(data.characters)
      ? data.characters
        .filter((c) => c && CHARACTER_FIELDS.some((f) => String(c[f] ?? '').trim()))
    ...
    // 立绘：导入 PNG 卡时把卡面登记进来（`{ 角色名: { card: 'rp-cards/x.png' } }`），
    // 界面在「没生成过立绘」时用它显示。生成出来的立绘仍然是临时预览，不落这里。
 ...
 *   条目正文……
 *
 * - **`##` 开一个新条目**（单个 `#` 是文档标题，不参与解析，这样文件可以有正常的一级标题）；
 * - `<!-- keys: ... -->` 可指定触发词（逗号、顿号或竖线分隔）、`constant`（常驻）、
 *   `order: N`（越大越优先）、`prob: 0-100`（触发概率）；
 ...
 *
 * @returns {{ active: object[], dropped: object[], matchedByScan: number }}
 */
function activateLore(entries, corpus, { sessionId, turn, maxEntries = LORE_MAX_ENTRIES, budgetChars = LORE_BUDGET_CHARS } = {}) {
  const text = String(corpus ?? '');
  const constants = [];
  const matched = [];
  ...
  return { active, dropped, matchedByScan: matched.length };
}
 ...
 */
function buildTurnContext(session, corpus, { sessionId, turn } = {}) {
  const parts = [];
  ...
  if (entries.length) {
    const { active, dropped } = activateLore(entries, corpus, { sessionId, turn });
...
const PARTY_FIELDS = ['character', 'status', 'inventory', 'conditions', 'goal'];

/** 状态字段在注入文本里的显示名。改了 STATE_FIELDS 记得同步这里（有测试兜底）。 */
const STATE_LABELS = {
  scene: '场景', time: '时间', location: '地点', present: '在场', clues: '线索',
  character: '角色', status: '状态', inventory: '持有', conditions: '伤病/影响', goal: '目标',
};
...
const rpTools = [
  ...
  // ---- 生成工具：状态追踪（伤势 / 物品 / 关系 / 伏笔）----
  {
    ...
    parameters: {
      ...
      party: {
        type: 'array',
        description: '队伍成员状态，每项 {character, status, inventory, conditions, goal}。只列**你要更新的人**；传空数组即清空全部。',
      },
      flags: {
        type: 'object',
  ...
  // ---- 生成工具：世界书按需补读 ----
  {
    ...
    parameters: {
      action: { type: 'string', description: '"find"（默认）按关键词/标题取条目；"list" 只列目录；"template" 在工作区生成一份带示例的世界书模板文件（已存在则不动）' },
 ...
 *
 * 分两层：
 * - `macroVars`：sessionId → Map<name, disposer>，记录**本进程里已经注册过的变量**（别再重复注册，宿主会抛）；
...
/** 某个会话此刻「已注册、可以安全留在文本里」的变量名集合（供 neutralizeMustache 判断）。 */
function registeredMacroNames(sessionId) {
  const set = new Set(macroVars.get(normalizeSessionId(sessionId))?.keys() ?? []);
  set.add('user');           // user 永远注册（值有全局默认）
  for (const name of AUTO_MACROS) set.add(name);   // time/date… 永远注册，值当场算
  return set;
}
...
function buildStandingText(session, extra = {}) {
  ...
  if (campaignName) parts.push(`战役：${campaignName}`);
...
function installStandingPrompt(ctx, opts = {}) {
  try {
    ...
    // 会话 id 直接取作用域自带的身份（agent 作用域下 ctx.agent.id 就是本会话），**不猜**
    const sessionIdOfCtx = () => {
      try {
        const id = normalizeSessionId(ctx?.agent?.id);
        if (id && id !== 'default') return id;
      } catch { /* 忽略 */ }
      return normalizeSessionId(opts.sessionId ?? '');
    };
    sys.section({
    ...
    sys.context({ name: TURN_CONTEXT, order: 20, text: '（本轮尚无世界书条目命中。）' });
    ...
    const selfSessionId = sessionIdOfCtx();
    /**
     * 当前会话 id：优先取作用域**此刻**的身份，取不到才用挂载时记下的那个。
     *
     * 为什么不能只用挂载时的值：路由与装配用的都是 `sessionIdOfCtx()` 的实时值，
     * 两处规则不一致就会出现「宏表写进了 A、provider 却去 B 里找」的空值
     * （测试里把 agent.id 换一个就复现了；生产里 id 是稳定的，但保持一致更不易错）。
     */
    const currentSessionId = () => sessionIdOfCtx() || selfSessionId;
    const providerFor = (name) => () => {
      const table = macroValueCache.get(currentSessionId()) ?? {};
      if (name === 'user') {
    ...
    const registerVar = (name) => {
      ...
      if (!id || !MACRO_NAME_RE.test(name)) return false;
      if (!macroVars.has(id)) macroVars.set(id, new Map());
      const owned = macroVars.get(id);
    ...
    // 挂载时先把 user + 当前宏表的键注册上，并允许全局路由通过 macroRegistrars 回调进来补注册
    macroRegistrars.set(selfSessionId, registerVar);
    const cfgAtMount = loadStyles();
    const initial = loadSession(selfSessionId).macros ?? {};
    macroValueCache.set(selfSessionId, mergedMacros(cfgAtMount, initial));
    registerVar('user');
    ...
    const off = ctx.on('system-prompt/assemble', async (assembly, context, next) => {
      ...
      const before = Array.isArray(result?.sections)
        ...
        : [];
      try {
        const sessionId = sessionIdOfCtx();
        ...
        // 路由侧要能按这个会话 id 回调进来补注册（键用实时 id，和上面保持一致）
        macroRegistrars.set(sessionId, registerVar);
        for (const name of Object.keys(macroValueCache.get(sessionId) ?? {})) registerVar(name);
        refreshSessionMacros(sessionId, session.macros ?? {});
        const known = registeredMacroNames(sessionId);
        const sections = Array.isArray(result?.sections) ? result.sections.slice() : [];

        const text = buildStandingText(session, { loreFile: sessionLorePath(sessionId)?.file, macros: known });
        if (text) {
...
function writeStandingProbe(before, after, sessionId = '') {
  try {
    ...
    const info = {
      ...
      loreInjectedChars: typeof turnCtx?.text === 'string' && turnCtx.text !== '（本轮尚无世界书条目命中。）'
        ? turnCtx.text.length : 0,
  ...
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        if (request.method === 'POST') {
          ...
          try {
            ...
            sendJson(response, 200, {
              ...
              keyed: entries.filter((e) => Array.isArray(e.keys) && e.keys.length).length,
              entries: (() => { const budget = { left: LORE_VIEW_TOTAL_CHARS }; return entries.slice(0, 400).map((e) => loreEntryView(e, budget)); })(),
              truncated: entries.length > 400,
              note: '只有命中的条目才进每轮上下文；文件可直接编辑，DM 也能用 read/write 维护。',
            });
          } catch (error) {
            sendJson(response, 400, { ok: false, error: String(error?.message ?? error) });
          }
        ...
        try {
          if (!workspace) {
            sendJson(response, 200, { ok: true, exists: false, file: null, entries: [], total: 0, hint: '拿不到这个会话的工作区目录' });
            return;
          }
          ...
          sendJson(response, 200, {
            ...
            entries: entries.slice(0, 400).map((e) => loreEntryView(e, budget)),
            truncated: entries.length > 400,
            note: '只有命中的条目才进每轮上下文；文件可直接编辑，DM 也能用 read/write 维护。',
          });
        } catch (error) {
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          if (isCharacter && !session.campaign?.name) session.campaign = { ...(session.campaign ?? {}), name: incoming.name };
          session.portraits = {
            ...(session.portraits && typeof session.portraits === 'object' ? session.portraits : {}),
            // card = 卡库内的相对路径（界面用它拼 /rp-tools/card-image 的 URL）；
            // file = 复制到工作区的那一份（工作区自带这张卡，DM 也能直接看）
            // 故事书没有人物可挂卡面，但仍把卡面复制进工作区（files.image），只是不登记立绘。
            ...(isCharacter && rel ? { [incoming.name]: { card: rel, file: files.relative.image } } : {}),
          };
          ...
          sendJson(response, 200, {
            ...
            path: rel,
            name: incoming.name,
            files: { ...files.relative, abs: { world: files.worldFile, markdown: files.mdFile, json: files.jsonFile, image: files.imageFile } },
            ...
            previousWorldChars,
            character: incoming,
            // false = 故事书：没建人物（界面用这句提示用户「人物请手动/SM 自己建」）
