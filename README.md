...
**保存只有一个入口**：页面最上面那颗「保存」把这一页的改动一起写好 —— 卡库配置 + 第三方工具
放行名单（两次请求，两个都是必要的；放行名单改完要换 DM 会话或重进才生效）。「恢复默认」同在最上面，
也是两件事一起做（卡库配置回出厂 + 放行名单回 `GLOBAL_TOOLS_DEFAULT`，后者由宿主给，界面不自己抄）。
第三方工具管理那一节**没有自己的按钮**（原来的「保存 / 全部放行 / 全部取消 / 恢复默认」与「手填工具名」
输入框整排删了）—— 两个保存入口会让人不确定「我改的到底存了没」。
...
它带的工具名，图片/生图插件带 🖼 标识。**勾完点页面最上面那颗「保存」**（这一节没有自己的按钮）。
...
映射表（`lib/global-tools-defaults.js` 的 `GLOBAL_TOOL_SOURCES`）。**映射表按插件一条一条登记，
没有「其它」那种大杂烩组** —— 已登记的是 `dsh-image-gen` / `dsh-genui` / `宿主内置 · 联网搜索` /
`dsh-mcp` / `dsh-lite-memory`（记忆·日历·定时合成一条）/ `dsh-updater-npm` / `dsh-find-plugin`。
映射表没覆盖到的才会落到兜底组「**宿主内置 · 其它**」（正常情况是空的；它比「其它」说得清，
又不至于挂着一句给维护者看的「请补映射表」）：装了新插件的工具落到那儿，**就往映射表加一条**
（正则 + 显示名）即可，界面不用改；也**不按名字前缀乱猜** —— 猜错会让用户以为自己看的是某个插件。

**清单是两层取并集**：`tools.schemas()` 不带 scope 只给**全局层**，而宿主内置的
`web_search` / `web_fetch` 是注册在**会话作用域**的 —— 只看全局层会把一直在用的内置工具
标成「未注册」（用户报的就是这句：*web_search 是内置的*）。所以插件记一个**样本 agent**
（只存引用、不序列化它的字段；样本来自 `agent/created` 与每轮的 `system-prompt/assemble`
的 `context.agent`，拿不到就退回纯全局视图），清单 = 全局层 ∪ **会话作用域里已经点过名的**那几个，
并滤掉本插件自己的 `rp_*`。

> ⚠️ 会话作用域那一份**必须按放行名单过滤**：样本 agent 实测就是「用户当前那个对话」的 agent，
> 它的作用域里有 `read` / `write` / `glob` / `pwsh` / `todo_write` 这 20 多个**自己的**工具 ——
> 照单全收会让设置页凭空多出二十几行「宿主内置 · 其它」。那张表的语义是「全局注册的工具 /
> 要放行给 DM 的第三方工具」，agent 的原生工具不属于它。

**「本机没注册」不再单独占一组**：配置里有、但两层注册表里都查不到的名字会**混进它
该在的那一组**，并在组上点名 `未注册 xxx` —— 同一个插件的工具不会被劈成两处。
