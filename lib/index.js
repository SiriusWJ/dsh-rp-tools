...
// 底线少一项就是「DM 卡片渲染当场坏掉且不报错」）。改名单只改那个文件。
import { GLOBAL_TOOLS_BASE, GLOBAL_TOOLS_ALLOW_DEFAULT, GLOBAL_TOOLS_ALLOW_MAX, TOOL_NAME_RE, normalizeGlobalToolsAllow } from './global-tools-defaults.js';
...
function defaultConfig() {
  return {
    ...
    // dm 会话要放行的**额外**全局工具（在 GLOBAL_TOOLS_BASE 之上追加）。
    // 典型就是宿主的生图工具；换生图插件时改这里即可，不必动预设。
    globalToolsAllow: [...GLOBAL_TOOLS_ALLOW_DEFAULT],
  };
}

/** 设置页/路由共用：报告当前的额外放行名单 + 底线（界面要能把底线显示成「固定勾选」）。 */
function globalToolsAllowView(cfg) {
  return {
    base: [...GLOBAL_TOOLS_BASE],
    allow: normalizeGlobalToolsAllow(cfg?.globalToolsAllow),
    defaults: [...GLOBAL_TOOLS_ALLOW_DEFAULT],
...
function loadStyles() {
  ...
  // 老配置迁移：DM 会话放行的全局工具（1.15.0 起可配置）。**缺字段 = 出厂默认**，
  // 这样老用户的盘上没这个键时也能拿到生图工具；显式给空数组才是「一个都不额外放行」。
  if (!Object.prototype.hasOwnProperty.call(cfg, 'globalToolsAllow')) {
    cfg.globalToolsAllow = [...GLOBAL_TOOLS_ALLOW_DEFAULT];
  } else {
...
/** 粗判「这个工具像是生图/图片相关的」——只用来给界面排序与默认勾选，不做任何强制。 */
function looksLikeImageTool(name) {
  return /(image|img|picture|photo|draw|paint|illustrat|caption|vision)/i.test(String(name ?? ''));
}
...
const rpTools = [
  ...
  // ---- 全局配置（卡库根目录 / 默认宏列表）----
  {
    ...
    parameters: {
      ...
      global_tools: {
        type: 'string',
        description: '配合 action:"set_global_tools"：要额外放行给 DM 会话的**全局**工具名，逗号分隔'
          + '（默认 `generate_image,edit_image` = 宿主的生图工具）。这些名字不在白名单里时 DM 看不见它们，'
          + '配图就无工具可用。传空串 = 只留预设底线（render_ui / validate_dsh_ui / web_search）。'
    ...
    execute: async (args) => {
      ...
      return {
        ...
        lines: [
          ...
          `放行给 DM 会话的全局工具：底线 ${toolView.base.join(' / ')}`
            + `${toolView.allow.length ? ` + 额外 ${toolView.allow.join(' / ')}` : '（没有额外放行 —— DM 看不到生图工具）'}`,
          available === null
  ...
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        // GET  ：列出「本机全部全局工具」+ 当前的额外放行名单（设置页据此渲染勾选框）
        // POST ：写入额外放行名单（持久化到 styles.json，dm-filter 会读它）
          ...
          try {
            ...
            sendJson(response, 200, {
              ...
              available: all === null ? null : all.map((name) => ({
                ...
                // 底线上的工具由预设固定放行，界面显示成「固定」且不可取消
                locked: GLOBAL_TOOLS_BASE.includes(name),
        ...
        try {
          ...
          if (!Array.isArray(raw)) { sendJson(response, 400, { ok: false, error: 'allow 必须是字符串数组（要额外放行的全局工具名）' }); return; }
          const cfg = loadStyles();
