...
import { buildImport, cardToMarkdown, buildOpeningPrompt, DEFAULT_USER_LABEL } from './card-import.js';
...
/** 世界书条目的界面表示（列表用；详情另外按标题取，避免把整本书塞进一次响应）。 */
function loreEntryView(e) {
  return {
    title: e.title,
    keys: e.keys,
    constant: e.constant,
      ...
      handler: async (request, response) => {
        ...
        if (request.method === 'POST') {
          ...
          try {
            ...
            sendJson(response, 200, {
              ...
              entries: entries.slice(0, 400).map(loreEntryView),
        ...
        try {
          ...
          if (wantTitle !== null) {
            ...
            sendJson(response, 200, {
              ok: true,
              entry: { ...loreEntryView(found), body: found.body, chars: found.body.length },
            });
            return;
          }
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: (request, response) => {
        ...
        try {
          ...
          const { character, world, stats, summary, placeholders } = got.built;
          sendJson(response, 200, {
            ok: true,
            path: rel,
            name: character.name,
