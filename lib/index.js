...
function sessionFile(sessionId) {
  return join(SESSIONS_DIR, `${String(sessionId).replace(/[^\w.-]/g, '_')}.json`);
}
...
function saveSession(session) {
  try {
    ...
    writeFileSync(sessionFile(session.sessionId), JSON.stringify({ ...session, updatedAt: new Date().toISOString() }, null, 2) + '\n', 'utf8');
    return true;
  } catch { return false; }
}
...
function markDmSession(sessionId, presetName) {
  try {
    ...
    index[String(sessionId)] = { preset: String(presetName ?? 'dm'), at: new Date().toISOString() };
      ...
      handler: async (request, response) => {
        ...
        if (request.method === 'GET') {
          ...
          try {
            ...
            sendJson(response, 200, {
              ok: true,
              isDm: Boolean(dmIndex[sessionId]) || session.preset === 'dm',
              preset: dmIndex[sessionId]?.preset ?? session.preset ?? '',
              session,
            });
          } catch (error) {
            sendJson(response, 500, { ok: false, error: String(error?.message ?? error) });
