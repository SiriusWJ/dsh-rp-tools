...
/** 时间类：卡里拿不到真实值，删掉。 */
const TIME_MACROS = /^(time|date|timezone|localtime|weekday|isotime)$/i;
 ...
 */
export function resolvePlaceholders(text, { userLabel = DEFAULT_USER_LABEL, charLabel = '', keepMacros = true, macros } = {}) {
  const counts = {};
  ...
  out = out.replace(/\{\{\s*([^{}]{0,80}?)\s*\}\}/g, (_m, rawName) => {
    ...
    if (TIME_MACROS.test(name)) { bump('{{time}}'); return ''; }
    ...
    // 名字不合法（`{{random:1,10}}`、`{{//注释}}`、带空格…）：去掉花括号保留文字
    bump('其它 {{…}}');
    return name;
  });
...
export function discoverMacros(text) {
  ...
  return [...found.entries()].map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count);
}
 ...
 */
export function buildImport(decoded, { maxEntries, maxChars, userLabel, charLabel, macros, keepMacros = true } = {}) {
  const data = decoded?.data ?? {};
  ...
  const fix = (text) => {
    const r = resolvePlaceholders(text, { userLabel, charLabel: char, macros, keepMacros });
 ...
 */
export function cardToMarkdown(decoded, { maxChars = 400000, userLabel, charLabel, keepMacros = true, macros } = {}) {
  const data = decoded?.data ?? {};
  const char = String(charLabel ?? '').trim() || String(data?.name ?? '').trim();
  // 全文导出也要展开占位符 + 中文化属性标签：
  // DM 会 read 这个文件，`{{user}}` 与 `gender: Female` 留在里面都是谜语
  const fix = (t) => localizeAttributes(resolvePlaceholders(t, { userLabel, charLabel: char, keepMacros, macros }).text).text;
  const out = [
