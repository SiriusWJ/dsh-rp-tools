...
function loadLoreEntries(sessionId) {
  ...
  const cwd = sessionCwd.get(id);
  if (!cwd) return [];
  const file = join(cwd, LORE_FILE_NAME);
  try {
...
const rpTools = [
  ...
  // ---- 生成工具：世界书按需补读 ----
  {
    name: 'rp_lore',
    description: [
      '读取本会话世界书（工作区根目录的 `' + LORE_FILE_NAME + '`）里的条目。',
    ...
    execute: async (args, exec) => {
      ...
      const file = cwd ? join(cwd, LORE_FILE_NAME) : '';
      const action = String(args.action ?? 'find');

      // 生成模板：让 DM 一键拿到正确格式，不用用户手记
      if (action === 'template') {
        if (!cwd) return { ok: false, file, total: 0, lines: [], note: `读不到本会话工作区目录，无法写入 ${LORE_FILE_NAME}` };
        if (existsSync(file)) {
      ...
      if (!entries.length) {
        return {
          ...
          note: cwd
            ? `世界书为空：本会话工作区里还没有 ${LORE_FILE_NAME}。可以让 DM 调 rp_lore(action:"template") 生成一份模板。`
  ...
  // ---- 生成工具：会话级设置（世界 / 前缀 / 风格备注 / 战役名 / 会话默认风格）----
  {
    ...
    description: [
      ...
      `世界书是本会话工作区根目录下的 \`${LORE_FILE_NAME}\`，只有被对话触发的条目才进上下文（省大量 token）。`,
...
/** 导入到工作区的子目录名。 */
const CARD_IMPORT_DIR = 'rp-cards';
...
/** 把一次导入的结果落盘到工作区（世界书 + 卡全文 + 卡面 + 卡 JSON）。 */
function writeImportFiles(workspace, { root, rel, built, decoded, name, userLabel, chosenGreeting }) {
  const dir = join(workspace, CARD_IMPORT_DIR);
  mkdirSync(dir, { recursive: true });
  const slug = cardSlug(rel, name);

  const worldFile = join(workspace, LORE_FILE_NAME);
  ...
  try {
    ...
    imageRel = `${CARD_IMPORT_DIR}/${slug}.png`;
  ...
  return {
    ...
    imageRel,
    relative: {
      world: LORE_FILE_NAME,
      markdown: `${CARD_IMPORT_DIR}/${slug}.md`,
      opening: `${CARD_IMPORT_DIR}/${slug}.opening.md`,
      json: `${CARD_IMPORT_DIR}/${slug}.json`,
      image: imageRel,
    },
  };
}
 ...
 * 中和 ST / 宿主风格的 `{{…}}` 双花括号。
 *
 * 为什么必须做：宿主会对 system 段做变量插值，残留的 `{{宏}}` 会被当成未定义变量处理而抛错。
 * 而我们的世界设定 / 风格触发词里出现 `{{user}}`、`{{char}}` 这类写法完全可能（ST 生态的习惯写法）。
 * 换成全角花括号：既不会触发插值，肉眼也仍能读出来。
 */
 ...
 */
function buildStandingText(session) {
  if (!session) return '';
  const parts = [];
...
function writeStandingProbe(before, after, sessionId = '') {
  try {
    ...
    const info = {
      ...
      loreFile: sessionCwd.get(sid) ? join(sessionCwd.get(sid), LORE_FILE_NAME) : null,
      ...
      handler: (request, response) => {
        ...
        try {
          ...
          if (wanted && wanted !== 'default') {
            ...
            const messages = typeof live?.deriveMessages === 'function' ? live.deriveMessages() : [];
            const standing = buildStandingText(session);
            ...
            sendJson(response, 200, {
              ...
              loreFile: sessionCwd.get(wanted) ? join(sessionCwd.get(wanted), LORE_FILE_NAME) : null,
      ...
      handler: async (request, response) => {
        ...
        if (request.method === 'POST') {
          ...
          try {
            ...
            const dir = resolveWorkspaceDir(sid, body.workspace);
            if (!dir) { sendJson(response, 400, { ok: false, error: '拿不到这个会话的工作区目录' }); return; }
            const file = join(dir, LORE_FILE_NAME);
        ...
        try {
          ...
          const file = join(workspace, LORE_FILE_NAME);
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          const files = writeImportFiles(workspace, {
            ...
            userLabel: cardUserLabel(cfg), chosenGreeting: chosen,
          });
