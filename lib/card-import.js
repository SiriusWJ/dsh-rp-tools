 ...
 * 这张卡是不是 **DM（旁白 / 主持人）卡**？
 *
 * 为什么要单独判：卡库里有一类卡不是「某个角色」，而是**给模型自己看的跑团规则**
 * —— 名字就是模型的名字（`lust Adventure`），描述通篇第三人称讲「它会怎么叙述」，
 * 开场白是对玩家说「给我一个故事背景，我来给你建世界」。
 * 按角色卡导入的话，这些规则会变成一条**名叫 DM 的角色卡**，既污染角色索引，
 * 又跟真正的 persona 抢话语权（用户实测：第一条角色卡里写的全是 DM 预设）。
 *
 * 判据（刻意要求**两个弱信号**，避免把正常角色卡误判）：
 * - 强信号（任一即算）：「你是一名 DM/GM/主持人」「扮演所有 NPC」「Dungeon/Game Master」
 * - 弱信号 A：文本自称主持人/旁白/叙述者/语言模型，或抱怨「AI 的语气」
 * - 弱信号 B：第三人称写「<卡名> will …」（英文 DM 卡的典型写法）
 * - 弱信号 C：开场白用「带你玩 / 给你建世界 / 给我一个故事背景」的口吻对玩家说话
 *
 * 真事故那张（`lust Adventure`）A+B+C 全中；正常角色卡（「{{char}}是{{user}}的师兄」）
 * 一个都不中；只在关系里提一句「旁白」的角色卡只中 A → 不算。
 */
export function isDmCardData(data) {
  const name = String(data?.name ?? '').trim();
  const text = [
    data?.description, data?.personality, data?.behavior, data?.speech,
    data?.scenario, data?.first_mes, data?.mes_example, data?.creator_notes, data?.relations,
  ].map((v) => String(v ?? '')).join('\n');
  if (!text.trim()) return false;

  // 强信号：明说自己就是主持人 / 要扮演所有 NPC
  if (/(扮演所有\s*(NPC|npc|角色|人物)|playing all NPCs|you are (the )?(game|dungeon) ?master|you are an? ?(DM|GM)\b|你是一名.{0,12}(DM|GM|主持人|游戏主持|地下城主)|(Dungeon|Game)\s?Master)/i.test(text)) return true;

  const roleSignal = /(\bDM\b|\bGM\b|主持人|旁白|叙述者|叙事者|语言模型|\bAI\b.{0,6}(语气|助手|assistant))/i.test(text);
  ...
  return [roleSignal, nameWill, gmTalk].filter(Boolean).length >= 2;
}
...
export function dmCardToPrompt(data, { fix } = {}) {
  ...
  const rows = [
    ['角色定位', cut(data?.description ?? data?.personality, 4000)],
    ['语气与叙述', cut(data?.speech, 4000)],
    ['行为准则', cut(data?.behavior ?? data?.personality, 6000)],
...
export function buildImport(decoded, { maxEntries, maxChars, userLabel, charLabel, macros, keepMacros = true, seed = '' } = {}) {
  ...
  // DM（旁白）卡同理：它的「角色」其实是 DM 自己的规则，要进**本会话 DM 设定**，
  // 不能建成一条名叫 DM 的角色卡（用户实测到过这种脏数据）。
  const isDmCard = isDmCardData(data);
  const isCharacterCard = !isDmCard && hasCharacterFields(data);
  const dmPrompt = isDmCard ? dmCardToPrompt(data, { fix }) : '';
  ...
  const world = [
    String(data?.name ?? '').trim()
      ? `${isDmCard ? '【DM 卡】' : isCharacterCard ? '【角色卡】' : '【故事书】'}${data.name}`
      : '',
  ...
  const summary = [
    isDmCard
      ? `DM（旁白）卡：${cardLabel} —— 已写进本会话的 **DM 设定**，没有建成角色卡`
      : isCharacterCard ? `角色卡：${character.name}` : `故事书：${cardLabel}（卡里没有角色描述/性格，不建角色卡）`,
  ...
  return {
    character,
    // 这张卡到底有没有角色：false = 故事书（书名为 name，角色在正文里）或 DM 卡。
    // 调用方据此决定**要不要**把它写进会话人物表（别把书名/模型名当人物）。
    isCharacterCard,
    // DM（旁白）卡：调用方把 `dmPrompt` 写进会话配置的 `dm.prompt`（见 /rp-tools/card-import）
    isDmCard,
    dmPrompt,
...
/**
 * 「整理设定」的指令正文（开局随开场指令一起发；已开局的会话可以让面板再发一次）。
 *
 * 为什么需要它：导入是**按规则解码**的 —— 卡里那些「当前进度 / 前情提要 / 物品清单」
 * 也会被当成世界书条目导进来（无 keys 的还会被补成 `constant`），既每轮占上下文，
 * 又是**会过期**的信息。规则判断力有限（我们已经用类别标签 + 空壳过滤挡了一层），
 * 而 DM 手里有卡全文、也在局内 —— 让它做一次整理最省，且不需要额外起一个 agent。
 */
 ...
 */
export function buildOpeningPrompt(imported, { worldFile, cardFile, imageRel, greeting, greetingFile, greetingCount = 0, macros, tidy = true } = {}) {
  ...
  // 设定整备：规则解码出来的世界书/角色卡需要 DM 收一次尾（用户要求「开局第一轮提示 DM 完善」）
  if (tidy) {
